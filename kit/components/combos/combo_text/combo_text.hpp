...
namespace Cherry
{
    ...
    namespace Kit
    {
        inline std::shared_ptr<Component> ComboText(const std::string &label, std::vector<std::string>* list, int default_index = 0)
        {
        ...
        inline std::shared_ptr<Component> ComboText(const Cherry::Identifier &identifier, const std::string &label, std::vector<std::string>* list, int default_index = 0)
        {
            if (identifier.string() == "__inline")
            {
                auto new_title =  std::make_shared<Components::ComboText>(Components::ComboText(identifier, label, list, default_index));
                new_title->Render();
                return new_title;
            }
            ...
            else
            {
                ...
                auto new_title = Application::CreateComponent<Components::ComboText>(Components::ComboText(identifier, label, list, default_index));
                new_title->Render();
                return new_title;
            }
