...
namespace Cherry {
  namespace Components {
    class ProgressBarPercentage : public Component {
     public:
      ProgressBarPercentage(const Cherry::Identifier &id, float *percentage) : Component(id), m_Percentage(percentage) {
        // Identifier
        SetIdentifier(id);
      }
  ...
  // End-User API
  namespace Kit {
    inline Component &ProgressBarPercentage(const Identifier &identifier, float *percentage) {
      return CherryApp.PushComponent<Cherry::Components::ProgressBarPercentage>(identifier, percentage);
    }

    inline Component &ProgressBarPercentage(float *percentage) {
      return Cherry::Kit::ProgressBarPercentage(
          Application::GenerateUniqueID(percentage, "ProgressBarPercentage"), percentage);
    }
  }  // namespace Kit
}  // namespace Cherry
