...
namespace Cherry {
     ...
     public:
      ...
      void Render() override {
        ...
        if (*activated) {
          ...
          CherryGUI::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(padding_x, padding_y));
          ...
          CherryGUI::SetNextWindowPos(ImVec2(window_pos.x + (main_window_size.x * 0.5f) - 300, window_pos.y + 150));

          CherryGUI::SetNextWindowSize(ImVec2(600, 0), ImGuiCond_Always);
          // ImGuiWindowFlags_NoMove
          if (CherryGUI::BeginPopupModal(
                  cached_title.c_str(), NULL, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoScrollbar)) {
            if (render_callback) {
              render_callback();
            }
            CherryGUI::EndPopup();
  ...
  namespace Kit {
    ...
    ModalSimple(const std::string &title, bool *activated, const std::function<void()> &render_callback = []() { }) {
      ...
      auto button = Application::CreateAnonymousComponent<Components::ModalSimple>(
          Components::ModalSimple(anonymous_id, title, activated, render_callback));
    ...
    inline std::shared_ptr<Component> ModalSimple(
        const Cherry::Identifier &identifier,
        const std::string &title,
        bool *activated,
        const std::function<void()> &render_callback = []() { }) {
      if (identifier.string() == "__inline") {
        auto new_title = Application::CreateAnonymousComponent<Components::ModalSimple>(
            Components::ModalSimple(identifier, title, activated, render_callback));
      ...
      if (existing_title) {
        ...
        auto new_title = Application::CreateComponent<Components::ModalSimple>(
            Components::ModalSimple(identifier, title, activated, render_callback));
        CherryApp.PushCurrentComponent(new_title);
        new_title->RenderWrapper();
        CherryApp.PopCurrentComponent();
