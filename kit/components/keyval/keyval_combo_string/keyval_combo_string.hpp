  ...
  namespace Kit {
    ...
    KeyValComboString(const std::string &label, std::vector<std::string> *list, int default_selected_index = 0) {
      ...
      if (existing) {
        ...
        auto button = Application::CreateAnonymousComponent<Components::KeyValComboString>(
            Components::KeyValComboString(anonymous_id, label, list, default_selected_index));
        button->Render();
        return button;
      }
    }

    inline std::shared_ptr<Component> KeyValComboString(
