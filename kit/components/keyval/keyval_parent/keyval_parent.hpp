...
namespace Cherry {
      ...
      void Render() override {
        ...
        if (std::stoi(parent->GetData("renderedColumn")) == 0) {
          ...
          if (CherryGUI::TableGetColumnIndex() == 0) {
            ...
            CherryGUI::PushStyleColor(ImGuiCol_HeaderHovered, ImVec4(1, 1, 1, 0.1f));
            CherryGUI::PushStyleColor(ImGuiCol_HeaderActive, ImVec4(1, 1, 1, 0.15f));

            bool selected = CherryGUI::Selectable(
                ("##fullrow_hoverzone_" + GetIdentifier().string()).c_str(),
                false,
          ...
          CherryGUI::TableSetBgColor(ImGuiTableBgTarget_CellBg, IM_COL32(170, 170, 170, 40));
        ...
        if (m_IsOpen && std::stoi(parent->GetData("renderedColumn")) == 1) {
          ...
          for (auto &child_comp : m_Rows) {
            ...
            for (int column = 0; column < std::stoi(parent->GetProperty("columns_number")); column++) {
  ...
  namespace Kit {
    inline Component &
    KeyValParent(const Identifier &identifier, const std::string &label, const std::vector<Component> &rows = {}) {
      return CherryApp.PushComponent<Cherry::Components::KeyValParent>(identifier, label, rows);
    }

    inline Component &KeyValParent(const std::string &label, const std::vector<Component> &rows = {}) {
      return Cherry::Kit::KeyValParent(Application::GenerateUniqueID(label, rows, "KeyValParent"), label, rows);
    }

    inline Component &KeyValParent(
        const Identifier &identifier,
        const std::string &label,
        bool default_opened = false,
        const std::vector<Component> &rows = {}) {
      return CherryApp.PushComponent<Cherry::Components::KeyValParent>(identifier, label, rows, default_opened);
    }

    inline Component &
    KeyValParent(const std::string &label, bool default_opened = false, const std::vector<Component> &rows = {}) {
      return Cherry::Kit::KeyValParent(
          Application::GenerateUniqueID(label, rows, "KeyValParent"), label, default_opened, rows);
    }
  }  // namespace Kit
