...
namespace Cherry {
  namespace Components {
    class KeyValColorHex : public Component {
     public:
      KeyValColorHex(const Cherry::Identifier &id, const std::string &label, std::string *value)
          : Component(id),
            m_Value(value) {
        // Identifier
        SetIdentifier(id);
      ...
      void Render() override {
        ...
        if (std::stoi(parent->GetData("renderedColumn")) == 0) {
          ...
          if (CherryGUI::BeginPopup(popupID.c_str())) {
            ...
            ImGuiColorEditFlags picker_flags = ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_DisplayRGB |
                                               ImGuiColorEditFlags_DisplayHSV | ImGuiColorEditFlags_DisplayHex;

            if (CherryGUI::ColorPicker4("##picker", (float *)&tempColorVec4, picker_flags)) {
  ...
  // End-User API
  namespace Kit {
    inline Component &KeyValColorHex(const Identifier &identifier, const std::string &label, std::string *value) {
      return CherryApp.PushComponent<Cherry::Components::KeyValColorHex>(identifier, label, value);
    }

    inline Component &KeyValColorHex(const std::string &label, std::string *value) {
      return Cherry::Kit::KeyValColorHex(Application::GenerateUniqueID(label, value, "KeyValColorHex"), label, value);
    }
