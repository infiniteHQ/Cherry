...
namespace Cherry {
  namespace Components {
    class KeyValInteger : public Component {
     public:
      KeyValInteger(const Cherry::Identifier &id, const std::string &label, int *value) : Component(id), m_Value(value) {
        // Identifier
        SetIdentifier(id);
        ...
        if (parent) {
          if (std::stoi(parent->GetData("renderedColumn")) == 0) {
            ...
            if (m_Value) {
              ...
              CherryGUI::InputInt(
                  Label.c_str(), m_Value, std::stoi(GetProperty("step")), std::stoi(GetProperty("step_fast")));
            } else {
  ...
  // End-User API
  namespace Kit {
    inline Component &KeyValInteger(const Identifier &identifier, const std::string &label, int *value) {
      return CherryApp.PushComponent<Cherry::Components::KeyValInteger>(identifier, label, value);
    }

    inline Component &KeyValInteger(const std::string &label, int *value) {
      return Cherry::Kit::KeyValInteger(Application::GenerateUniqueID(label, value, "KeyValInteger"), label, value);
    }
