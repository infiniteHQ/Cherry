...
namespace Cherry
{
    ...
    namespace Kit
    {
        inline std::shared_ptr<Component> KeyValVector3(const std::string &label, float *x_value, float *y_value, float *z_value)
        {
            // Inline component
            auto anonymous_id = Application::GenerateUniqueID(label, x_value, y_value, z_value);
            ...
            else
            {
                ...
                auto button = Application::CreateAnonymousComponent<Components::KeyValVector3>(Components::KeyValVector3(anonymous_id, label, x_value, y_value, z_value));
                button->Render();
                return button;
            }
        }

        inline std::shared_ptr<Component> KeyValVector3(const Cherry::Identifier &identifier, const std::string &label, float *x_value, float *y_value, float *z_value)
        {
