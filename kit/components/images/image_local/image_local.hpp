...
namespace Cherry {
  namespace Components {
    class ImageLocal : public Component {
     public:
      ImageLocal(const Cherry::Identifier &id, const std::string &image_path, float x_size, float y_size) : Component(id) {
        // Identifier
        SetIdentifier(id);
  ...
  namespace Kit {
    inline Component &
    ImageLocal(const Identifier &identifier, const std::string &image_path, float x_size = 50.0f, float y_size = 50.0f) {
      return CherryApp.PushComponent<Cherry::Components::ImageLocal>(identifier, image_path, x_size, y_size);
    }

    inline Component &ImageLocal(const std::string &image_path, float x_size = 50.0f, float y_size = 50.0f) {
      return Cherry::Kit::ImageLocal(
          Application::GenerateUniqueID(image_path, x_size, y_size, "ImageLocal"), image_path, x_size, y_size);
    }
  }  // namespace Kit
}  // namespace Cherry
