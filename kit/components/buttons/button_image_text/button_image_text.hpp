  ...
  namespace Kit {
    inline std::shared_ptr<Component> ButtonImageText(const std::string &label, const std::string &image_path) {
      ...
      if (existing) {
        ...
        auto button = Application::CreateAnonymousComponent<Components::ButtonImageText>(
            Components::ButtonImageText(anonymous_id, label, image_path));
        button->Render();
        return button;
      }
    }
    ...
    ButtonImageText(const Cherry::Identifier &identifier, const std::string &label, const std::string &image_path) {
      if (identifier.string() == "__inline") {
        auto new_button =
            std::make_shared<Components::ButtonImageText>(Components::ButtonImageText(identifier, label, image_path));
        new_button->Render();
        return new_button;
      }
      ...
      if (existing_button) {
        ...
        auto new_button = Application::CreateComponent<Components::ButtonImageText>(
            Components::ButtonImageText(identifier, label, image_path));
        new_button->Render();
        return new_button;
      }
