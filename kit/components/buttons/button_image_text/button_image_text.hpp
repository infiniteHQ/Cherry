...
namespace Cherry {
  ...
  namespace Kit {
    inline Component &
    ButtonImageText(const Identifier &identifier, const std::string &label, const std::string &image_path) {
      return CherryApp.PushComponent<Cherry::Components::ButtonImageText>(identifier, label, image_path);
    }

    inline Component &ButtonImageText(const std::string &label, const std::string &image_path) {
      return Cherry::Kit::ButtonImageText(
          Application::GenerateUniqueID(label, image_path, "ButtonImageText"), label, image_path);
    }
  }  // namespace Kit
