  ...
  namespace Kit {
    inline std::shared_ptr<Component> ButtonTextUnderline(const std::string &label) {
      ...
      if (existing) {
        ...
        auto button = Application::CreateAnonymousComponent<Components::ButtonTextUnderline>(
            Components::ButtonTextUnderline(anonymous_id, label));
        button->Render();
        return button;
      }
    }
    ...
    inline std::shared_ptr<Component> ButtonTextUnderline(const Cherry::Identifier &identifier, const std::string &label) {
      if (identifier.string() == "__inline") {
        auto new_button =
            std::make_shared<Components::ButtonTextUnderline>(Components::ButtonTextUnderline(identifier, label));
        new_button->Render();
        return new_button;
      }
      ...
      if (existing_button) {
        ...
        auto new_button = Application::CreateComponent<Components::ButtonTextUnderline>(
            Components::ButtonTextUnderline(identifier, label));
        new_button->Render();
        return new_button;
      }
