  ...
  namespace Kit {
    inline std::shared_ptr<Component> ButtonText(const std::string &label) {
      ...
      if (existing) {
        ...
        auto button =
            Application::CreateAnonymousComponent<Components::ButtonText>(Components::ButtonText(anonymous_id, label));
        button->Render();
        return button;
      }
    }

    inline std::shared_ptr<Component> ButtonText(const Cherry::Identifier &identifier, const std::string &label) {
      if (identifier.string() == "__inline") {
        auto new_button = std::make_shared<Components::ButtonText>(Components::ButtonText(identifier, label));
        new_button->Render();
        return new_button;
      }
