...
namespace Cherry {
    ...
    class ButtonImageDropdown : public Component {
     public:
      ButtonImageDropdown(
          ...
          : Component(id),
            m_DropdownCallback(dropdown_callback) {
        // Identifier
        SetIdentifier(id);
      ...
      void Render() override {
        ...
        bool isClicked = CherryGUI::ImageButtonWithTextWithIcon(image_texture, texture, Label.c_str(), size, size);
  ...
  // End-User API
  namespace Kit {
    inline std::shared_ptr<Component> ButtonImageDropdown(
        const std::string &image_path,
        const std::function<void()> &dropdown_callback = []() { }) {
      ...
      if (existing) {
        ...
        auto button = Application::CreateAnonymousComponent<Components::ButtonImageDropdown>(
            Components::ButtonImageDropdown(anonymous_id, image_path, dropdown_callback));
        button->Render();
        return button;
      }
    }
    ...
    inline std::shared_ptr<Component> ButtonImageDropdown(
      ...
      if (identifier.string() == "__inline") {
        auto new_button = Application::CreateAnonymousComponent<Components::ButtonImageDropdown>(
            Components::ButtonImageDropdown(identifier, image_path, dropdown_callback));
        new_button->Render();
        return new_button;
      }
      ...
      if (existing_button) {
        ...
        auto new_button = Application::CreateComponent<Components::ButtonImageDropdown>(
            Components::ButtonImageDropdown(identifier, image_path, dropdown_callback));
        new_button->Render();
        return new_button;
      }
