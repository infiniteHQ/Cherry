...
namespace Cherry {
    ...
    class ButtonImageTextDropdown : public Component {
     public:
      ...
      void Render() override {
        ...
        if (CherryGUI::ImageButtonWithTextWithIcon(image_texture, texture, Label.c_str(), size, size)) {
  ...
  // End-User API
  namespace Kit {
    inline std::shared_ptr<Component> ButtonImageTextDropdown(const std::string &label, const std::string &image_path) {
      ...
      if (existing) {
        ...
        auto button = Application::CreateAnonymousComponent<Components::ButtonImageTextDropdown>(
            Components::ButtonImageTextDropdown(anonymous_id, label, image_path));
        button->Render();
        return button;
      }
    }
    ...
    ButtonImageTextDropdown(const Cherry::Identifier &identifier, const std::string &label, const std::string &image_path) {
      if (identifier.string() == "__inline") {
        auto new_button = std::make_shared<Components::ButtonImageTextDropdown>(
            Components::ButtonImageTextDropdown(identifier, label, image_path));
        new_button->Render();
        return new_button;
      }
      ...
      if (existing_button) {
        ...
        auto new_button = Application::CreateComponent<Components::ButtonImageTextDropdown>(
            Components::ButtonImageTextDropdown(identifier, label, image_path));
        new_button->Render();
        return new_button;
      }
