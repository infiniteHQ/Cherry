...
namespace Cherry {
  ...
  namespace Kit {
    ...
    inline Component &ButtonTextDropdown(
        ...
        const std::string &label,
        const std::function<void()> &dropdown_callback = []() { }) {
      return CherryApp.PushComponent<Cherry::Components::ButtonTextDropdown>(identifier, label, dropdown_callback);
    }
    ...
    inline Component &ButtonTextDropdown(
      ...
      return Cherry::Kit::ButtonTextDropdown(
          Application::GenerateUniqueID(label, dropdown_callback, "ButtonTextDropdown"), label, dropdown_callback);
    }
