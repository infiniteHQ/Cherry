...
namespace Cherry {
     ...
     public:
      ...
      void Render() override {
        ...
        ImTextureID second_texture =
            Application::Get().GetCurrentRenderedWindow()->get_texture(GetProperty("second_image_path"));
  ...
  namespace Kit {
    ...
    ButtonImageTextImage(const std::string &label, const std::string &image_path, const std::string &second_image_path) {
      auto anonymous_id = Application::GenerateUniqueID(label, image_path, second_image_path);
      auto existing = Application::GetAnonymousComponent(anonymous_id);
      if (existing) {
        existing->Render();
        return existing;
      } else {
        auto title = Application::CreateAnonymousComponent<Components::ButtonImageTextImage>(
            Components::ButtonImageTextImage(anonymous_id, label, image_path, second_image_path));
        title->Render();
        return title;
      }
    }

    inline std::shared_ptr<Component> ButtonImageTextImage(
        const Cherry::Identifier &identifier,
        const std::string &label,
        const std::string &image_path,
        const std::string &second_image_path) {
      if (identifier.string() == "__inline") {
        auto new_title = Application::CreateAnonymousComponent<Components::ButtonImageTextImage>(
            Components::ButtonImageTextImage(identifier, label, image_path, second_image_path));
        new_title->Render();
        return new_title;
      }
      ...
      if (existing_title) {
        ...
        auto new_title = Application::CreateComponent<Components::ButtonImageTextImage>(
            Components::ButtonImageTextImage(identifier, label, image_path, second_image_path));
        new_title->Render();
        return new_title;
      }
