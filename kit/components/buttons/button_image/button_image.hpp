...
namespace Cherry {
  ...
  namespace Kit {
    ...
    inline Component &ButtonImage(const Cherry::Identifier &identifier, const std::string &image_path) {
      return CherryApp.PushComponent<Cherry::Components::ButtonImage>(identifier, image_path);
    }

    inline Component &ButtonImage(const std::string &image_path) {
      return Cherry::Kit::ButtonImage(Application::GenerateUniqueID(image_path, "ButtonImage"), image_path);
    }
  }  // namespace Kit
