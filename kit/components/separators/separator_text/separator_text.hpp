...
namespace Cherry {
  namespace Components {
    class SeparatorText : public Component {
     public:
      SeparatorText(const Cherry::Identifier &id, const std::string &label) : Component(id) {
        // Identifier
        SetIdentifier(id);
  ...
  // End-User API
  namespace Kit {
    inline Component &SeparatorText(const Identifier &identifier, const std::string &label) {
      return CherryApp.PushComponent<Cherry::Components::SeparatorText>(identifier, label);
    }

    inline Component &SeparatorText(const std::string &label) {
      return Cherry::Kit::SeparatorText(Application::GenerateUniqueID(label), label);
    }
