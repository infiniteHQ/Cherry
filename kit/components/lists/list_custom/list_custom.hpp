...
namespace Cherry {
  ...
  // End-User API
  namespace Kit {
    inline Component &ListCustom(
        const Identifier &identifier,
        const std::string &label,
        const std::vector<Component> &values,
        int default_selected = 0) {
      return CherryApp.PushComponent<Cherry::Components::ListCustom>(identifier, label, values, default_selected);
    }

    inline Component &ListCustom(const std::string &label, const std::vector<Component> &values, int default_selected = 0) {
      return Cherry::Kit::ListCustom(
          Application::GenerateUniqueID(label, values, default_selected, "ListCustom"), label, values, default_selected);
    }
