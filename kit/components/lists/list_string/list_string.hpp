...
namespace Cherry {
  ...
  // End-User API
  namespace Kit {
    inline Component &ListString(
        const Identifier &identifier,
        const std::string &label,
        const std::vector<std::string> &values,
        int default_selected = 0) {
      return CherryApp.PushComponent<Cherry::Components::ListString>(identifier, label, values, default_selected);
    }

    inline Component &
    ListString(const std::string &label, const std::vector<std::string> &values, int default_selected = 0) {
      return Cherry::Kit::ListString(
          Application::GenerateUniqueID(label, values, default_selected, "ListString"), label, values, default_selected);
    }
