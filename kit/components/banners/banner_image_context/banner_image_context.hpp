...
namespace Cherry {
     ...
     public:
      ...
      void Render() override {
        ...
        if (!image.empty() && std::filesystem::exists(image)) {
          drawList->AddImage(
              Cherry::GetTexture(image), cursorPos, ImVec2(cursorPos.x + squareSize.x, cursorPos.y + squareSize.y));
        } else {
          drawList->AddImage(
              Cherry::GetTexture(image), cursorPos, ImVec2(cursorPos.x + squareSize.x, cursorPos.y + squareSize.y));
        }
  ...
  namespace Kit {
    inline Component &BannerImageContext(
        ...
        const std::string &title = "",
        const std::string &description = "") {
      return CherryApp.PushComponent<Cherry::Components::BannerImageContext>(identifier, image_path, title, description);
    }
    ...
    inline Component &BannerImageContext(
      ...
      return CherryApp.PushComponent<Cherry::Components::BannerImageContext>(
          identifier, image_path, title, description, width, height, onClickedCallback);
    }
    ...
    inline Component &BannerImageContext(
      ...
      return Cherry::Kit::BannerImageContext(
          Application::GenerateUniqueID(image_path, title, description, "BannerImageContext"),
          image_path,
          title,
          description);
    }
    ...
    inline Component &BannerImageContext(
        ...
        const std::function<void()> &onClickedCallback = []() { }) {
      return Cherry::Kit::BannerImageContext(
          Application::GenerateUniqueID(
              width, height, image_path, title, description, onClickedCallback, "BannerImageContext"),
          width,
          height,
          image_path,
          title,
