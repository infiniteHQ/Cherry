...
namespace Cherry {
  ...
  namespace Kit {
    inline std::shared_ptr<Component> TitleFive(const std::string &label) {
      ...
      auto button = Application::CreateAnonymousComponent<Components::TitleFive>(Components::TitleFive(anonymous_id, label));
      button->Render();
      return button;
    }
    ...
    inline std::shared_ptr<Component> TitleFive(const Cherry::Identifier &identifier, const std::string &label) {
      if (identifier.string() == "__inline") {
        auto new_title =
            Application::CreateAnonymousComponent<Components::TitleFive>(Components::TitleFive(identifier, label));
        new_title->Render();
        return new_title;
      }
