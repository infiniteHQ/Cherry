...
namespace Cherry {
  ...
  // End-User API
  namespace Kit {
    inline Component &TitleTwo(const Identifier &identifier, const std::string &label) {
      return CherryApp.PushComponent<Cherry::Components::TitleTwo>(identifier, label);
    }

    inline Component &TitleTwo(const std::string &label) {
      return Cherry::Kit::TitleTwo(Application::GenerateUniqueID(label, "TitleTwo"), label);
    }
