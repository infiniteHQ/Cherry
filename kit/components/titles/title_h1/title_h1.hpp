...
namespace Cherry {
  namespace Components {
    class TitleOne : public Component {
     public:
      TitleOne(const Cherry::Identifier &id, const std::string &label) : Component(id) {
        // Identifier
        SetIdentifier(id);
  ...
  // End-User API
  namespace Kit {
    inline std::shared_ptr<Component> TitleOne(const std::string &label) {
      ...
      auto button = Application::CreateAnonymousComponent<Components::TitleOne>(Components::TitleOne(anonymous_id, label));
      button->Render();
      return button;
    }

    inline std::shared_ptr<Component> TitleOne(const Cherry::Identifier &identifier, const std::string &label) {
      if (identifier.string() == "__inline") {
        auto new_title =
            Application::CreateAnonymousComponent<Components::TitleOne>(Components::TitleOne(identifier, label));
        new_title->Render();
        return new_title;
      }
