...
inline std::shared_ptr<Component> WebView(const std::string &url) {
  ...
  if (existing) {
    ...
    auto title = Application::CreateAnonymousComponent<Components::WebView>(
        Components::WebView(anonymous_id, url));
    title->Render();
    return title;
  }
}

inline std::shared_ptr<Component> WebView(const Cherry::Identifier &identifier,
                                          const std::string &url) {
  if (identifier.string() == "__inline") {
    auto new_title = std::make_shared<Components::WebView>(
        Components::WebView(identifier, url));
    new_title->Render();
    return new_title;
  }
  ...
  if (existing_title) {
    ...
    auto new_title = Application::CreateComponent<Components::WebView>(
        Components::WebView(identifier, url));
    new_title->Render();
    return new_title;
  }
