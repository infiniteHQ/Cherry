...
namespace Cherry {
  ...
  namespace Kit {
    inline Component &HeaderText(
      ...
      return CherryApp.PushComponent<Cherry::Components::HeaderText>(identifier, label, render_content);
    }

    inline Component &HeaderText(const std::string &label, const std::function<void()> &render_content = []() { }) {
      return Cherry::Kit::HeaderText(
          Application::GenerateUniqueID(label, render_content, "HeaderText"), label, render_content);
    }
