...
namespace Cherry {
    ...
    class WidgetFolder : public Component {
     public:
      ...
      void Render() override {
        ...
        if (isClicked || (CherryGUI::IsItemHovered() && CherryGUI::IsMouseDoubleClicked(ImGuiMouseButton_Right))) {
  ...
  // End-User API
  namespace Kit {
    inline Component &WidgetFolder(
        const Identifier &identifier,
        const std::string &color = "#CC9900",
        const float &size_x = 150.0f,
        const float &size_y = 150.0f) {
      return CherryApp.PushComponent<Cherry::Components::WidgetFolder>(identifier, color, size_x, size_y);
    }
    ...
    WidgetFolder(const std::string &color = "#CC9900", const float &size_x = 150.0f, const float &size_y = 150.0f) {
      return Cherry::Kit::WidgetFolder(
          Application::GenerateUniqueID(color, size_x, size_y, "WidgetFolder"), color, size_x, size_y);
    }
  }  // namespace Kit
