...
namespace Cherry {
  ...
  // End-User API
  namespace Kit {
    inline Component &TextWrapped(const Identifier &identifier, const std::string &label) {
      return CherryApp.PushComponent<Cherry::Components::TextWrapped>(identifier, label);
    }

    inline Component &TextWrapped(const std::string &label) {
      return Cherry::Kit::TextWrapped(Application::GenerateUniqueID(label, "TextWrapped"), label);
    }
