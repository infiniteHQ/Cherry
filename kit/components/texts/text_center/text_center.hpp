...
namespace Cherry {
  namespace Components {
    class TextCenter : public Component {
     public:
      TextCenter(const Cherry::Identifier &id, const std::string &label) : Component(id) {
        // Identifier
        SetIdentifier(id);
  ...
  namespace Kit {
    ...
    inline Component &TextCenter(const Identifier &identifier, const std::string &label) {
      return CherryApp.PushComponent<Cherry::Components::TextCenter>(identifier, label);
    }

    inline Component &TextCenter(const std::string &label) {
      return Cherry::Kit::TextCenter(Application::GenerateUniqueID(label, "TextCenter"), label);
    }
