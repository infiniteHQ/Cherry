    ...
    class BlockVerticalCustom : public Component {
     public:
      BlockVerticalCustom(
          ...
          : Component(id),
            m_RenderCallbacks(renderCallbacks),
      ...
      void Render() override {
        ...
        if (CherryGUI::BeginChild(
                "RenderCallbacks", cachedSize, false, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse)) {
          CherryGUI::InvisibleButton(GetIdentifier().string().c_str(), cachedSize);

          if (CherryGUI::IsWindowHovered(ImGuiHoveredFlags_AllowWhenBlockedByActiveItem) &&
        ...
        drawList->AddRect(
            pos, ImVec2(pos.x + cachedSize.x, pos.y + cachedSize.y), border_color, cached_radius, 0, border_size);
      }
  ...
  namespace Kit {
    inline std::shared_ptr<Component> BlockVerticalCustom(const std::vector<std::function<void()>> &renderCallbacks) {
      ...
      if (existing) {
        ...
        auto button = Application::CreateAnonymousComponent<Components::BlockVerticalCustom>(
            Components::BlockVerticalCustom(anonymous_id, renderCallbacks));
        button->Render();
        return button;
      }
    }

    inline std::shared_ptr<Component> BlockVerticalCustom(
        const std::function<void()> &onClickedCallback = []() { },
        const float &width = 260.0f,
        const float &height = 110.0f,
        const std::vector<std::function<void()>> &renderCallbacks = {},
      ...
      if (existing) {
        ...
        auto button = Application::CreateAnonymousComponent<Components::BlockVerticalCustom>(
            Components::BlockVerticalCustom(anonymous_id, renderCallbacks, width, height, onClickedCallback));
        button->Render();
        return button;
      }
    }
    ...
    inline std::shared_ptr<Component> BlockVerticalCustom(
      ...
      switch (identifier.property()) {
        case IdentifierProperty::Inline: {
          auto new_title = Application::CreateAnonymousComponent<Components::BlockVerticalCustom>(
              Components::BlockVerticalCustom(identifier, renderCallbacks));
          new_title->Render();
          return new_title;
          break;
        }
      ...
      if (existing_title) {
        ...
        auto new_title = Application::CreateComponent<Components::BlockVerticalCustom>(
            Components::BlockVerticalCustom(identifier, renderCallbacks));
        new_title->Render();
        return new_title;
      }
    ...
    inline std::shared_ptr<Component> BlockVerticalCustom(
        Cherry::IdentifierProperty prop,
        const std::vector<std::function<void()>> &renderCallbacks) {
      auto anonymous_id = Application::GenerateUniqueID(renderCallbacks);
      switch (prop) {
        case IdentifierProperty::Inline: {
    ...
    inline std::shared_ptr<Component> BlockVerticalCustom(
      ...
      if (existing_title) {
        ...
        // Create the object if not exist
        auto new_title = Application::CreateComponent<Components::BlockVerticalCustom>(
            Components::BlockVerticalCustom(anonymous_id, renderCallbacks, width, height));
        new_title->Render();
        return new_title;
      }
      return existing_title;
    ...
    inline std::shared_ptr<Component> BlockVerticalCustom(
      ...
      fullParams.emplace_back(onClickedCallback);
      fullParams.emplace_back(width);
      fullParams.emplace_back(height);
      fullParams.emplace_back(renderCallbacks);
    ...
    inline std::shared_ptr<Component> BlockVerticalCustom(
      ...
      switch (identifier.property()) {
        case IdentifierProperty::Inline: {
          auto new_title = Application::CreateAnonymousComponent<Components::BlockVerticalCustom>(
              Components::BlockVerticalCustom(identifier, renderCallbacks, width, height, onClickedCallback));
          new_title->Render();
          return new_title;
          break;
        }
      ...
      if (existing_title) {
        ...
        // Create the object if not exist
        auto new_title = Application::CreateComponent<Components::BlockVerticalCustom>(
            Components::BlockVerticalCustom(identifier, renderCallbacks, width, height));
        new_title->Render();
        return new_title;
      }
      return existing_title;
