...
namespace Cherry {
  namespace Components {
    class NodeAreaMaker : public Component {
     public:
      NodeAreaMaker(const Cherry::Identifier &id, const std::string &label, int width, int height) : Component(id) {
        // Identifier
        SetIdentifier(id);
      ...
      void BuildNodes() {
        ...
        m_NodeEngine->m_Links.push_back(Link(
            m_NodeEngine->GetNextLinkId(), m_NodeEngine->m_Nodes[5].Outputs[0].ID, m_NodeEngine->m_Nodes[6].Inputs[0].ID));
        m_NodeEngine->m_Links.push_back(Link(
            m_NodeEngine->GetNextLinkId(), m_NodeEngine->m_Nodes[5].Outputs[0].ID, m_NodeEngine->m_Nodes[7].Inputs[0].ID));

        m_NodeEngine->m_Links.push_back(Link(
            m_NodeEngine->GetNextLinkId(), m_NodeEngine->m_Nodes[14].Outputs[0].ID, m_NodeEngine->m_Nodes[15].Inputs[0].ID));
      }
  ...
  // End-User API
  namespace Kit {
    inline Component &NodeAreaMaker(
        const Identifier &identifier,
        const std::string &label,
        int width,
        int height,
        const std::vector<Component> &node_array = {}) {
      return CherryApp.PushComponent<Cherry::Components::NodeAreaMaker>(identifier, label, width, height);
    }

    inline Component &
    NodeAreaMaker(const std::string &label, int width, int height, const std::vector<Component> &node_array = {}) {
      return Cherry::Kit::NodeAreaMaker(
          Application::GenerateUniqueID(label, width, height, "NodeAreaMaker"), label, width, height);
    }
