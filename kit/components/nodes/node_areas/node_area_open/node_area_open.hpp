  ...
  /* void OnStart() override
   {
       ...
       config.SaveNodeSettings = [](ed::NodeId nodeId, const char* data, size_t size, ed::SaveReasonFlags reason, void*
   userPointer) -> bool
       {
           auto self = static_cast<Example*>(userPointer);
       ...
       node = SpawnComment();              ed::SetNodePosition(node->ID, ImVec2(112, 576)); ed::SetGroupSize(node->ID,
   ImVec2(384, 154)); node = SpawnComment();              ed::SetNodePosition(node->ID, ImVec2(800, 224));
  ...
  void ShowLeftPane(float paneWidth) {
    ...
    for (auto &node : m_Nodes) {
      ...
      if (CherryGUI::Selectable(
              (node.Name + "##" + std::to_string(reinterpret_cast<uintptr_t>(node.ID.AsPointer()))).c_str(), &isSelected)) {
        if (io.KeyCtrl) {
          if (isSelected)
...
namespace Cherry {
          ...
          for (auto &node : m_NodeEngine->m_Nodes) {
            ...
            if (!isSimple) {
              ...
              if (!node.SecondName.empty()) {
                CherryGUI::BeginHorizontal("horizontal_logo_and_text");  // Commence la ligne

                if (!node.LogoPath.empty()) {
                  CherryGUI::BeginVertical("logo_align");  // Pour centrer le logo verticalement
  ...
  // End-User API
  namespace Kit {
    inline std::shared_ptr<Component> NodeAreaOpen(
        const std::string &label,
        int width,
        int height,
        Cherry::NodeSystem::NodeContext *node_ctx,
      ...
      if (existing) {
        ...
        auto button = Application::CreateAnonymousComponent<Components::NodeAreaOpen>(
            Components::NodeAreaOpen(anonymous_id, label, width, height, node_ctx, graph));
        button->Render();
        return button;
      }
    }
    ...
    inline std::shared_ptr<Component> NodeAreaOpen(
      ...
      if (existing_button) {
        ...
        auto new_button = Application::CreateComponent<Components::NodeAreaOpen>(
            Components::NodeAreaOpen(identifier, label, width, height, node_ctx, graph));
        new_button->Render();
        return new_button;
      }
    }
