...
enum class LabelTooltipIcon { Plus, Cross, None };
...
struct Example {
  ...
  bool CanCreateLink(Pin *a, Pin *b) {
    if (!a || !b || a == b || a->Kind == b->Kind ||
        a->Format.m_TypeID != b->Format.m_TypeID || a->Node == b->Node)
      return false;
  ...
  void RefreshNodeGraphLinks() {
    ...
    for (const auto &conn : m_NodeGraph->m_Connections) {
      ...
      m_Links.emplace_back(Link(GetNextId(), pinA->ID, pinB->ID,
                                Cherry::HexToImColor(pinA->Format.m_Color)));
    }
  }
  ...
  Node *SpawnNode(Cherry::NodeSystem::NodeSchema *schema) {
    ...
    m_Nodes.emplace_back(GetNextId(), schema->GetLabel().c_str(),
                         Cherry::HexToImColor(schema->GetHexHeaderColor()));
...
*/
  ...
  void ShowStyleEditor(bool *show = nullptr) {
    ...
    CherryGUI::Spacing();
    CherryGUI::DragFloat4("Node Padding", &editorStyle.NodePadding.x, 0.1f,
                          0.0f, 40.0f);
    CherryGUI::DragFloat("Node Rounding", &editorStyle.NodeRounding, 0.1f, 0.0f,
                         40.0f);
    CherryGUI::DragFloat("Node Border Width", &editorStyle.NodeBorderWidth,
                         0.1f, 0.0f, 15.0f);
    CherryGUI::DragFloat("Hovered Node Border Width",
                         &editorStyle.HoveredNodeBorderWidth, 0.1f, 0.0f,
                         15.0f);
    CherryGUI::DragFloat("Hovered Node Border Offset",
                         &editorStyle.HoverNodeBorderOffset, 0.1f, -40.0f,
                         40.0f);
    CherryGUI::DragFloat("Selected Node Border Width",
                         &editorStyle.SelectedNodeBorderWidth, 0.1f, 0.0f,
                         15.0f);
    CherryGUI::DragFloat("Selected Node Border Offset",
                         &editorStyle.SelectedNodeBorderOffset, 0.1f, -40.0f,
                         40.0f);
    CherryGUI::DragFloat("Pin Rounding", &editorStyle.PinRounding, 0.1f, 0.0f,
                         40.0f);
    CherryGUI::DragFloat("Pin Border Width", &editorStyle.PinBorderWidth, 0.1f,
                         0.0f, 15.0f);
    CherryGUI::DragFloat("Link Strength", &editorStyle.LinkStrength, 1.0f, 0.0f,
                         500.0f);
    // ImVec2  SourceDirection;
    // ImVec2  TargetDirection;
    CherryGUI::DragFloat("Scroll Duration", &editorStyle.ScrollDuration, 0.001f,
                         0.0f, 2.0f);
    CherryGUI::DragFloat("Flow Marker Distance",
                         &editorStyle.FlowMarkerDistance, 1.0f, 1.0f, 200.0f);
    CherryGUI::DragFloat("Flow Speed", &editorStyle.FlowSpeed, 1.0f, 1.0f,
                         2000.0f);
    CherryGUI::DragFloat("Flow Duration", &editorStyle.FlowDuration, 0.001f,
                         0.0f, 5.0f);
    ...
    // float   PinArrowSize;
    // float   PinArrowWidth;
    CherryGUI::DragFloat("Group Rounding", &editorStyle.GroupRounding, 0.1f,
                         0.0f, 40.0f);
    CherryGUI::DragFloat("Group Border Width", &editorStyle.GroupBorderWidth,
                         0.1f, 0.0f, 15.0f);
  ...
  void ShowLeftPane(float paneWidth) {
    ...
    CherryGUI::GetWindowDrawList()->AddRectFilled(
        ...
        CherryGUI::GetCursorScreenPos() +
            ImVec2(paneWidth, CherryGUI::GetTextLineHeight()),
        ImColor(CherryGUI::GetStyle().Colors[ImGuiCol_HeaderActive]),
        CherryGUI::GetTextLineHeight() * 0.25f);
    CherryGUI::Spacing();
    CherryGUI::SameLine();
    ...
    for (auto &node : m_Nodes) {
      ...
      auto iconPanelPos =
          start + ImVec2(paneWidth - CherryGUI::GetStyle().FramePadding.x -
                             CherryGUI::GetStyle().IndentSpacing -
                             saveIconWidth - restoreIconWidth -
                             CherryGUI::GetStyle().ItemInnerSpacing.x * 1,
                         (CherryGUI::GetTextLineHeight() - saveIconHeight) / 2);
      CherryGUI::GetWindowDrawList()->AddText(
          ImVec2(iconPanelPos.x - textSize.x -
                     CherryGUI::GetStyle().ItemInnerSpacing.x,
                 start.y),
      ...
      if (node.SavedState.empty()) {
        ...
        if (CherryGUI::IsItemActive())
          drawList->AddImage(m_SaveIcon, CherryGUI::GetItemRectMin(),
                             CherryGUI::GetItemRectMax(), ImVec2(0, 0),
      ...
      if (!node.SavedState.empty()) {
        ...
        if (CherryGUI::IsItemActive())
          drawList->AddImage(m_RestoreIcon, CherryGUI::GetItemRectMin(),
                             CherryGUI::GetItemRectMax(), ImVec2(0, 0),
                             ImVec2(1, 1), IM_COL32(255, 255, 255, 96));
        else if (CherryGUI::IsItemHovered())
          drawList->AddImage(m_RestoreIcon, CherryGUI::GetItemRectMin(),
    ...
    CherryGUI::Text("Changed %d time%s", changeCount,
                    changeCount > 1 ? "s" : "");
    CherryGUI::Spring();
...
// mechanics for managing nodes, displaying them, handling connections, and
// interactions.
//
// If you want a more "ready-to-use" solution with a node builder, linker, and
// position manager, please check out "Cherry::NodeAreaMaker".
//
// In NodeAreaOpen, you need to specify: the array of compatible nodes and the
...
public:
  ...
  void Render() override {
    m_NodeEngine->m_HeaderBackground = Cherry::GetTexture(
        Cherry::GetPath("resources/base/blueprintbackground.png"));
    m_NodeEngine->m_SaveIcon =
        Cherry::GetTexture(Cherry::GetPath("resources/base/x.png"));
    m_NodeEngine->m_RestoreIcon =
        Cherry::GetTexture(Cherry::GetPath("resources/base/x.png"));

    NodeEditor::GetStyle().Colors[NodeEditor::StyleColor::StyleColor_Bg] =
        Cherry::HexToRGBA("#252525");
    ...
    ed::Begin("Node qsd");
    {
      ...
      for (auto &node : m_NodeEngine->m_Nodes) {
        ...
        if (!node.BackgroundColor.empty()) {
          ed::PushStyleColor(ed::StyleColor_NodeBg,
                             Cherry::HexToImColor(node.BackgroundColor));
        }

        if (!node.BorderColor.empty()) {
          ed::PushStyleColor(ed::StyleColor_NodeBorder,
                             Cherry::HexToImColor(node.BorderColor));
        }

        if (node.Status == Cherry::NodeSystem::NodeSchemaStatus::Depreciated) {
          ed::PushStyleColor(ed::StyleColor_NodeBg,
                             Cherry::HexToImColor("#908c0099"));
          ed::PushStyleColor(ed::StyleColor_NodeBorder,
                             Cherry::HexToImColor("#d8d100"));
        } else if (node.Status ==
                   Cherry::NodeSystem::NodeSchemaStatus::Obsolete) {
          ed::PushStyleColor(ed::StyleColor_NodeBg,
                             Cherry::HexToImColor("#89370499"));
          ed::PushStyleColor(ed::StyleColor_NodeBorder,
                             Cherry::HexToImColor("#d85200"));
        }
        ...
        if (!isSimple) {
          ...
          if (!node.SecondName.empty()) {
            ...
            CherryGUI::PushStyleColor(ImGuiCol_Text,
                                      Cherry::HexToRGBA(node.NameColor));
            CherryGUI::TextUnformatted(node.Name.c_str());
            CherryGUI::PopStyleColor();

            CherryGUI::GetFont()->Scale =
                Application::GetCurrentRenderedWindow()
                    ->m_Specifications.FontGlobalScale;
            CherryGUI::PopFont();

            CherryGUI::PushStyleColor(ImGuiCol_Text,
                                      Cherry::HexToRGBA(node.SecondNameColor));
          ...
          if (hasOutputDelegates) {
            ...
            for (auto &output : node.Outputs) {
              ...
              if (newLinkPin &&
                  !m_NodeEngine->CanCreateLink(newLinkPin, &output) &&
                  &output != newLinkPin)
        ...
        for (auto &output : node.Outputs) {
          ...
          auto alpha = CherryGUI::GetStyle().Alpha;
          if (newLinkPin && !m_NodeEngine->CanCreateLink(newLinkPin, &output) &&
              &output != newLinkPin)
            alpha = alpha * (48.0f / 255.0f);
      ...
      for (auto &node : m_NodeEngine->m_Nodes) {
        ...
        if (!node.Inputs.empty()) {
          ...
          if (newLinkPin && !m_NodeEngine->CanCreateLink(newLinkPin, &pin) &&
              &pin != newLinkPin)
            inputAlpha =
                (int)(255 * CherryGUI::GetStyle().Alpha * (48.0f / 255.0f));
        } else
          CherryGUI::Dummy(ImVec2(0, padding));
      ...
      for (auto &node : m_NodeEngine->m_Nodes) {
        ...
        if (ed::BeginGroupHint(node.ID)) {
          ...
          // CherryGUI::GetStyle().Alpha * 255);
          auto bgAlpha = static_cast<int>(CherryGUI::GetStyle().Alpha * 255);

          // CherryGUI::PushStyleVar(ImGuiStyleVar_Alpha, commentAlpha *
          // CherryGUI::GetStyle().Alpha);

          auto min = ed::GetGroupMin();
          // auto max = ed::GetGroupMax();

          CherryGUI::SetCursorScreenPos(
              min - ImVec2(-8, CherryGUI::GetTextLineHeightWithSpacing() + 4));
      ...
      if (!createNewNode) {
        if (ed::BeginCreate(ImColor(255, 255, 255), 2.0f)) {
          auto showLabel = [](const char *label, ImColor color,
                              LabelTooltipIcon icon = LabelTooltipIcon::None) {
            CherryGUI::SetCursorPosY(CherryGUI::GetCursorPosY() -
                                     CherryGUI::GetTextLineHeight());
            ...
            CherryGUI::SetCursorPos(CherryGUI::GetCursorPos() +
            ...
            drawList->AddRectFilled(rectMin,
                                    ImVec2(rectMax.x + iconSize.x, rectMax.y),
                                    color, size.y * 0.15f);
            ...
            if (startPin && endPin) {
              ...
              // }
                ...
                showLabel("Create Link", ImColor(32, 45, 32, 180),
                          LabelTooltipIcon::Plus);
                if (ed::AcceptNewItem(ImColor(128, 255, 128), 4.0f)) {
                  m_NodeEngine->m_Links.emplace_back(
                      Link(m_NodeEngine->GetNextId(), startPinId, endPinId));
        ...
        if (ed::BeginDelete()) {
          ...
          while (ed::QueryDeletedNode(&nodeId)) {
            if (ed::AcceptDeletedItem()) {
              auto id = std::find_if(
                  m_NodeEngine->m_Nodes.begin(), m_NodeEngine->m_Nodes.end(),
          ...
          while (ed::QueryDeletedLink(&linkId)) {
            if (ed::AcceptDeletedItem()) {
              auto id = std::find_if(
                  m_NodeEngine->m_Links.begin(), m_NodeEngine->m_Links.end(),
