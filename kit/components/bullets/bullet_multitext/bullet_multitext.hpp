...
namespace Cherry {
  namespace Components {
    class BulletMultiText : public Component {
     public:
      BulletMultiText(const Cherry::Identifier &id, const std::vector<std::string> &labels) : Component(id) {
        // Identifier
        SetIdentifier(id);
  ...
  // End-User API
  namespace Kit {
    inline Component &BulletMultiText(const Identifier &identifier, const std::vector<std::string> &labels) {
      return CherryApp.PushComponent<Cherry::Components::BulletMultiText>(identifier, labels);
    }

    inline Component &BulletMultiText(const std::vector<std::string> &labels) {
      return Cherry::Kit::BulletMultiText(Application::GenerateUniqueID(labels, "BulletMultiText"), labels);
    }
