...
namespace Cherry {
  ...
  // End-User API
  namespace Kit {
    inline std::shared_ptr<Component> NotificationButton(
        bool* activate,
        int timer_in_seconds = 8,
        const std::string& type = "info",
        const std::string& title = "Super Notification",
      ...
      if (existing) {
        ...
        auto button = Application::CreateAnonymousComponent<Components::NotificationButton>(Components::NotificationButton(
            anonymous_id, activate, timer_in_seconds, type, title, description, button_callback));
        button->Render();
        return button;
      }
    }
    ...
    inline std::shared_ptr<Component> NotificationButton(
      ...
      if (identifier.string() == "__inline") {
        auto new_button =
            Application::CreateAnonymousComponent<Components::NotificationButton>(Components::NotificationButton(
                identifier, activate, timer_in_seconds, type, title, description, button_callback));
        new_button->Render();
        return new_button;
      }
      ...
      if (existing_button) {
        ...
        auto new_button = Application::CreateComponent<Components::NotificationButton>(Components::NotificationButton(
            identifier, activate, timer_in_seconds, type, title, description, button_callback));
        new_button->Render();
        return new_button;
      }
