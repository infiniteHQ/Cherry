...
namespace Cherry
{
    namespace Components
    {
        ...
        {
        public:
            NotificationCustom(const Cherry::Identifier &id, bool *activate, int timer_in_seconds = 8, const std::string& type = "info", const std::string& title = "Super Notification", const std::function<void()>& render_callback= [](){})
                : Component(id), m_Activate(activate), m_TimerSeconds(timer_in_seconds), m_RenderCallback(render_callback)
            {
                // Identifier
                SetIdentifier(id);
    ...
    namespace Kit
    {
        inline std::shared_ptr<Component> NotificationCustom(bool *activate, int timer_in_seconds = 8, const std::string& type = "info", const std::string& title = "Super Notification", const std::function<void()>& render_callback= [](){})
        {
            ...
            {
                auto button = Application::CreateAnonymousComponent<Components::NotificationCustom>(Components::NotificationCustom(anonymous_id, activate, timer_in_seconds, type, title, render_callback));
                button->Render();
                return button;
            }
        }

        inline std::shared_ptr<Component> NotificationCustom(const Cherry::Identifier &identifier, bool *activate, int timer_in_seconds = 8, const std::string& type = "info", const std::string& title = "Super Notification", const std::function<void()>& render_callback= [](){})
        {
            if (identifier.string() == "__inline")
            {
                auto new_button = Application::CreateAnonymousComponent<Components::NotificationCustom>(Components::NotificationCustom(identifier, activate, timer_in_seconds, type, title, render_callback));
                new_button->Render();
                return new_button;
            }
            ...
            else
            {
                ...
                auto new_button = Application::CreateComponent<Components::NotificationCustom>(Components::NotificationCustom(identifier, activate, timer_in_seconds, type, title, render_callback));
                new_button->Render();
                return new_button;
            }
