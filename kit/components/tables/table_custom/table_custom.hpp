...
namespace Cherry {
  ...
  // End-User API
  namespace Kit {
    inline Component &TableCustom(
        const Identifier &identifier,
        const std::string &label,
        const std::vector<std::function<void(int)>> &rows = {},
        ImGuiTableFlags flags = 0) {
      return CherryApp.PushComponent<Cherry::Components::TableCustom>(identifier, label, rows, flags);
    }
    ...
    inline Component &TableCustom(
        ...
        const std::vector<std::function<void(int)>> &rows = {},
        ImGuiTableFlags flags = 0) {
      return Cherry::Kit::TableCustom(Application::GenerateUniqueID(label, rows, flags, "TableCustom"), label, rows, flags);
    }
