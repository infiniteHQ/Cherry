...
namespace Cherry {
  ...
  // End-User API
  namespace Kit {
    inline std::shared_ptr<Component> TableSimple(
        const std::string &label,
        const std::vector<std::shared_ptr<Component>> &rows = {},
        ImGuiTableFlags flags = 0) {
      // Inline component
      /*auto table = Application::CreateAnonymousComponent<Components::TableSimple>(Components::TableSimple(anonymous_id,
      label, rows, flags));
    ...
    inline std::shared_ptr<Component> TableSimple(
      ...
      if (existing_table) {
        ...
        auto new_table =
            Application::CreateComponent<Components::TableSimple>(Components::TableSimple(identifier, label, rows, flags));
