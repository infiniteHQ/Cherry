...
CHERRY_API const char *CalcWordWrapPositionA(ImFont *ptr, float scale,
                                             const char *text,
                                             const char *text_end,
                                             float wrap_width);
CHERRY_API void RenderChar(ImFont *ptr, ImDrawList *draw_list, float size,
                           ImVec2 pos, ImU32 col, ImWchar c);
CHERRY_API void RenderText(ImFont *ptr, ImDrawList *draw_list, float size,
                           ImVec2 pos, ImU32 col, const ImVec4 &clip_rect,
...
// Drawlist
CHERRY_API void AddLine(ImDrawList *ptr, const ImVec2 &p1, const ImVec2 &p2,
                        ImU32 col, float thickness = 1.0f);
CHERRY_API void
AddRect(ImDrawList *ptr, const ImVec2 &p_min, const ImVec2 &p_max, ImU32 col,
        float rounding = 0.0f, ImDrawFlags flags = 0,
        float thickness =
            1.0f); // a: upper-left, b: lower-right (== upper-left + size)
CHERRY_API void
AddRectFilled(ImDrawList *ptr, const ImVec2 &p_min, const ImVec2 &p_max,
              ImU32 col, float rounding = 0.0f,
              ImDrawFlags flags =
                  0); // a: upper-left, b: lower-right (== upper-left + size)
CHERRY_API void AddRectFilledMultiColor(ImDrawList *ptr, const ImVec2 &p_min,
                                        const ImVec2 &p_max, ImU32 col_upr_left,
                                        ImU32 col_upr_right,
                                        ImU32 col_bot_right,
                                        ImU32 col_bot_left);
CHERRY_API void AddQuad(ImDrawList *ptr, const ImVec2 &p1, const ImVec2 &p2,
                        const ImVec2 &p3, const ImVec2 &p4, ImU32 col,
                        float thickness = 1.0f);
CHERRY_API void AddQuadFilled(ImDrawList *ptr, const ImVec2 &p1,
                              const ImVec2 &p2, const ImVec2 &p3,
                              const ImVec2 &p4, ImU32 col);
CHERRY_API void AddTriangle(ImDrawList *ptr, const ImVec2 &p1, const ImVec2 &p2,
                            const ImVec2 &p3, ImU32 col,
                            float thickness = 1.0f);
CHERRY_API void AddTriangleFilled(ImDrawList *ptr, const ImVec2 &p1,
                                  const ImVec2 &p2, const ImVec2 &p3,
                                  ImU32 col);
CHERRY_API void AddCircle(ImDrawList *ptr, const ImVec2 &center, float radius,
                          ImU32 col, int num_segments = 0,
                          float thickness = 1.0f);
CHERRY_API void AddCircleFilled(ImDrawList *ptr, const ImVec2 &center,
                                float radius, ImU32 col, int num_segments = 0);
CHERRY_API void AddNgon(ImDrawList *ptr, const ImVec2 &center, float radius,
                        ImU32 col, int num_segments, float thickness = 1.0f);
CHERRY_API void AddNgonFilled(ImDrawList *ptr, const ImVec2 &center,
                              float radius, ImU32 col, int num_segments);
CHERRY_API void AddText(ImDrawList *ptr, const ImVec2 &pos, ImU32 col,
                        const char *text_begin, const char *text_end = NULL);
CHERRY_API void AddText(ImDrawList *ptr, const ImFont *font, float font_size,
...
CHERRY_API void
AddBezierCubic(ImDrawList *ptr, const ImVec2 &p1, const ImVec2 &p2,
               const ImVec2 &p3, const ImVec2 &p4, ImU32 col, float thickness,
               int num_segments = 0); // Cubic Bezier (4 control points)
CHERRY_API void
AddBezierQuadratic(ImDrawList *ptr, const ImVec2 &p1, const ImVec2 &p2,
...
CHERRY_API bool BeginMenuBar(
    float offsetY); // append to menu-bar of current window (requires
                    // ImGuiWindowFlags_MenuBar flag set on parent window).
CHERRY_API bool
BeginBottomBar(); // append to menu-bar of current window (requires
                  // ImGuiWindowFlags_MenuBar flag set on parent window).

CHERRY_API void EndMenuBar();
CHERRY_API void
EndBottomBar(); // only call EndMenuBar() if BeginMenuBar() returns true!
CHERRY_API bool
BeginMainMenuBar(); // create and append to a full screen menu-bar.
CHERRY_API void EndMainMenuBar();

// Legacy ImGui ui elements
CHERRY_API bool Button(const char *label,
                       const ImVec2 &size = ImVec2(0, 0)); // button
CHERRY_API bool ImageSizeButtonWithText(
    ImTextureID texId, float button_width, const char *label,
...
CHERRY_API bool
SmallButton(const char *label); // button with FramePadding=(0,0) to easily
                                // embed within text
...
CHERRY_API void ProgressBar(float fraction,
...
CHERRY_API bool TextButtonWithImageWithIcon(
    ImTextureID texIdFirst, ImTextureID texIdSecond, const char *label,
...
CHERRY_API bool ImageButtonWithTextWithIcon(
    ...
    const ImVec4 &tint_col = ImVec4(1, 1, 1, 1));
CHERRY_API bool
ImageButtonWithText(ImTextureID texId, const char *label,
...
CHERRY_API bool
BeginMenuImage(const char *label, ImTextureID *texture,
               bool enabled = true); // create a sub-menu entry. only call
                                     // EndMenu() if this returns true!
...
CHERRY_API bool
MenuItem(const char *label, const char *shortcut, ImTextureID texture,
         bool selected,
         bool enabled = true); // return true when activated + toggle
                               // (*p_selected) if p_selected != NULL
...
CHERRY_API void
EndPopup(); // did mouse button double-clicked? Same as
...
CHERRY_API void LabelTextV(const char *label, const char *fmt, va_list args)
    IM_FMTLIST(2);
...
CHERRY_API void SameLine(float offset_from_start_x = 0.0f,
                         float spacing = -1.0f);
...
CHERRY_API void BeginHorizontal(const char *str_id,
                                const ImVec2 &size = ImVec2(0, 0),
                                float align = -1.0f);
CHERRY_API void BeginHorizontal(const void *ptr_id,
                                const ImVec2 &size = ImVec2(0, 0),
                                float align = -1.0f);
CHERRY_API void BeginHorizontal(int id, const ImVec2 &size = ImVec2(0, 0),
                                float align = -1);
CHERRY_API void EndHorizontal();
CHERRY_API void BeginVertical(const char *str_id,
                              const ImVec2 &size = ImVec2(0, 0),
                              float align = -1.0f);
CHERRY_API void BeginVertical(const void *ptr_id,
                              const ImVec2 &size = ImVec2(0, 0),
                              float align = -1.0f);
CHERRY_API void BeginVertical(int id, const ImVec2 &size = ImVec2(0, 0),
                              float align = -1);
...
CHERRY_API bool IsMouseDragging(ImGuiMouseButton button,
...
CHERRY_API bool BeginCombo(const char *label,
                           const std::function<void()> &preview,
                           ImGuiComboFlags flags = 0);
CHERRY_API void
...
CHERRY_API bool BeginChild(const char *str_id,
                           const ImVec2 &size = ImVec2(0, 0),
...
CHERRY_API void ColorConvertRGBtoHSV(float r, float g, float b, float &out_h,
...
CHERRY_API bool TreeNode(const void *ptr_id, const char *fmt, ...)
    IM_FMTARGS(2); // "
CHERRY_API bool TreeNodeV(const char *str_id, const char *fmt, va_list args)
    IM_FMTLIST(2);
CHERRY_API bool TreeNodeV(const void *ptr_id, const char *fmt, va_list args)
    IM_FMTLIST(2);
CHERRY_API bool TreeNodeEx(const char *label, ImGuiTreeNodeFlags flags = 0);
CHERRY_API bool TreeNodeEx(const char *str_id, ImGuiTreeNodeFlags flags,
                           const char *fmt, ...) IM_FMTARGS(3);
CHERRY_API bool TreeNodeEx(const void *ptr_id, ImGuiTreeNodeFlags flags,
                           const char *fmt, ...) IM_FMTARGS(3);
CHERRY_API bool TreeNodeExV(const char *str_id, ImGuiTreeNodeFlags flags,
                            const char *fmt, va_list args) IM_FMTLIST(3);
CHERRY_API bool TreeNodeExV(const void *ptr_id, ImGuiTreeNodeFlags flags,
                            const char *fmt, va_list args) IM_FMTLIST(3);
CHERRY_API void
...
CHERRY_API bool Begin(const char *name, bool *p_open = NULL,
...
CHERRY_API bool InputText(const char *label, char *buf, size_t buf_size,
                          ...
                          void *user_data = NULL);
CHERRY_API bool InputTextWithImage(const char *label, ImTextureID texture,
                                   char *buf, size_t buf_size,
                                   ImGuiInputTextFlags flags = 0,
                                   ImGuiInputTextCallback callback = NULL,
                                   void *user_data = NULL);
CHERRY_API bool InputTextMultiline(const char *label, char *buf,
                                   size_t buf_size,
                                   const ImVec2 &size = ImVec2(0, 0),
                                   ImGuiInputTextFlags flags = 0,
                                   ImGuiInputTextCallback callback = NULL,
                                   void *user_data = NULL);
CHERRY_API bool InputTextWithHint(const char *label, const char *hint,
                                  char *buf, size_t buf_size,
                                  ImGuiInputTextFlags flags = 0,
                                  ImGuiInputTextCallback callback = NULL,
                                  void *user_data = NULL);
...
CHERRY_API bool InputColorFloat(const char *label, float *v,
                                ...
                                ImGuiInputTextFlags flags = 0);
CHERRY_API bool InputFloat2(const char *label, float v[2],
                            const char *format = "%.3f",
                            ImGuiInputTextFlags flags = 0);
CHERRY_API bool InputFloat3(const char *label, float v[3],
                            const char *format = "%.3f",
                            ImGuiInputTextFlags flags = 0);
CHERRY_API bool InputFloat4(const char *label, float v[4],
                            const char *format = "%.3f",
                            ImGuiInputTextFlags flags = 0);
CHERRY_API bool InputInt(const char *label, int *v, int step = 1,
                         int step_fast = 100, ImGuiInputTextFlags flags = 0);
CHERRY_API bool InputInt2(const char *label, int v[2],
                          ImGuiInputTextFlags flags = 0);
CHERRY_API bool InputInt3(const char *label, int v[3],
                          ImGuiInputTextFlags flags = 0);
CHERRY_API bool InputInt4(const char *label, int v[4],
                          ImGuiInputTextFlags flags = 0);
CHERRY_API bool InputDouble(const char *label, double *v, double step = 0.0,
                            double step_fast = 0.0, const char *format = "%.6f",
                            ImGuiInputTextFlags flags = 0);
CHERRY_API bool InputColorScalar(const char *label, ImGuiDataType data_type,
                                 ImVec4 color, void *p_data,
                                 const void *p_step = NULL,
                                 const void *p_step_fast = NULL,
                                 const char *format = NULL,
                                 ImGuiInputTextFlags flags = 0);
CHERRY_API bool InputScalar(const char *label, ImGuiDataType data_type,
                            void *p_data, const void *p_step = NULL,
                            const void *p_step_fast = NULL,
                            const char *format = NULL,
                            ImGuiInputTextFlags flags = 0);
CHERRY_API bool InputScalarN(const char *label, ImGuiDataType data_type,
...
#endif
CHERRY_API bool DragScalar(const char *label, ImGuiDataType data_type,
                           void *p_data, float v_speed, const void *p_min,
                           const void *p_max, const char *format, float power);
CHERRY_API bool DragScalarN(const char *label, ImGuiDataType data_type,
                            void *p_data, int components, float v_speed,
                            const void *p_min, const void *p_max,
                            const char *format, float power);
CHERRY_API bool SliderScalar(const char *label, ImGuiDataType data_type,
                             void *p_data, const void *p_min, const void *p_max,
                             const char *format, float power);
CHERRY_API bool SliderScalarN(const char *label, ImGuiDataType data_type,
...
CHERRY_API bool ColorEdit3(const char *label, float col[3],
                           ImGuiColorEditFlags flags = 0);
CHERRY_API bool ColorEdit4(const char *label, float col[4],
                           ImGuiColorEditFlags flags = 0);
...
CHERRY_API void RenderArrow(ImDrawList *draw_list, ImTextureID *texture,
                            ImVec2 pos, ImU32 col, ImGuiDir dir, float scale);
CHERRY_API void RenderArrow(ImDrawList *draw_list, ImVec2 pos, ImU32 col,
                            ImGuiDir dir, float scale = 1.0f);
CHERRY_API void RenderBullet(ImDrawList *draw_list, ImTextureID *texture,
                             ImVec2 pos, ImU32 col);
CHERRY_API void RenderBullet(ImDrawList *draw_list, ImVec2 pos, ImU32 col);
CHERRY_API void RenderCheckMark(ImDrawList *draw_list, ImVec2 pos, ImU32 col,
                                float sz);
CHERRY_API void RenderArrowPointingAt(ImDrawList *draw_list, ImVec2 pos,
                                      ImVec2 half_sz, ImGuiDir direction,
                                      ImU32 col);
CHERRY_API void RenderArrowDockMenu(ImDrawList *draw_list, ImVec2 p_min,
                                    float sz, ImU32 col);
CHERRY_API void RenderRectFilledRangeH(ImDrawList *draw_list,
                                       const ImRect &rect, ImU32 col,
                                       float x_start_norm, float x_end_norm,
                                       float rounding);
CHERRY_API void RenderRectFilledWithHole(ImDrawList *draw_list, ImRect outer,
...
DragInt(const char *label, int *v, float v_speed = 1.0f, int v_min = 0,
        ...
        ImGuiSliderFlags flags = 0); // If v_min >= v_max we have no bound
CHERRY_API bool DragInt2(const char *label, int v[2], float v_speed = 1.0f,
                         int v_min = 0, int v_max = 0,
                         const char *format = "%d", ImGuiSliderFlags flags = 0);
CHERRY_API bool DragInt3(const char *label, int v[3], float v_speed = 1.0f,
                         int v_min = 0, int v_max = 0,
                         const char *format = "%d", ImGuiSliderFlags flags = 0);
CHERRY_API bool DragInt4(const char *label, int v[4], float v_speed = 1.0f,
                         int v_min = 0, int v_max = 0,
                         const char *format = "%d", ImGuiSliderFlags flags = 0);
CHERRY_API bool DragIntRange2(const char *label, int *v_current_min,
...
CHERRY_API void Columns(int count = 1, const char *id = NULL,
                        bool border = true);
...
CHERRY_API float
GetColumnWidth(int column_index = -1); // get column width (in pixels).
...
CHERRY_API void TableSetupColumn(const char *label,
...
CHERRY_API bool BeginTable(const char *str_id, int column,
...
CHERRY_API void PushClipRect(const ImVec2 &clip_rect_min,
...
CHERRY_API void PushAllowKeyboardFocus(
