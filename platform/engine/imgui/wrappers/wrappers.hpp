...
bool BeginMenuBar(); // append to menu-bar of current window (requires
                     // ImGuiWindowFlags_MenuBar flag set on parent window).
bool BeginMenuBar(
    float offsetY);    // append to menu-bar of current window (requires
                       // ImGuiWindowFlags_MenuBar flag set on parent window).
bool BeginBottomBar(); // append to menu-bar of current window (requires
                       // ImGuiWindowFlags_MenuBar flag set on parent window).
...
bool SmallButton(const char *label); // button with FramePadding=(0,0) to easily
                                     // embed within text
...
void Bullet(); // draw a small circle + keep the cursor on the same line.
...
bool BeginMenuImage(const char *label, ImTextureID *texture,
                    bool enabled = true); // create a sub-menu entry. only call
                                          // EndMenu() if this returns true!
void EndMenu();
...
bool BeginPopupModal(
    ...
    ImGuiWindowFlags flags = 0); // return true if the modal is open, and you
                                 // can start outputting to it.
void EndPopup();                 // did mouse button double-clicked? Same as
...
float GetFontSize(); // get current font size (= height in pixels) of current
                     // font with current scale applied
ImVec2 GetFontTexUvWhitePixel(); // get UV coordinate for a while pixel, useful
...
void TextWrapped(const char *fmt, ...) IM_FMTARGS(
...
void LabelText(const char *label, const char *fmt, ...) IM_FMTARGS(
...
bool IsItemVisible(); // is the last item visible? (items may be out of sight
                      // because of clipping/scrolling)
bool IsItemEdited();  // did the last item modify its underlying value this
                     // frame? or was pressed? This is generally the same as the
                     // "bool" return value of many widgets.
bool IsItemActivated();   // was the last item just made active (item was
                          // previously inactive).
bool IsItemDeactivated(); // was the last item just made inactive (item was
                          // previously active). Useful for Undo/Redo patterns
                          // with widgets that requires continuous editing.
bool IsItemDeactivatedAfterEdit(); // was the last item just made inactive and
                                   ...
                                   // selected item).
bool IsItemToggledOpen(); // was the last item open state toggled? set by
                          // TreeNode().
...
ImVec2 GetItemRectMin();  // get upper-left bounding rectangle of the last item
                          // (screen space)
ImVec2 GetItemRectMax();  // get lower-right bounding rectangle of the last item
                          // (screen space)
...
float GetWindowDpiScale(); // get DPI scale currently associated to the current
                           // window's viewport.
ImVec2 GetWindowPos(); // get current window position in screen space (useful if
                       // you want to do your own drawing via the DrawList API)
ImVec2 GetWindowSize();  // get current window size
float GetWindowWidth();  // get current window width (shortcut for
                         // GetWindowSize().x)
float GetWindowHeight(); // get current window height (shortcut for
                         // GetWindowSize().y)
ImGuiViewport *
...
void SetNextWindowFocus(); // set next window to be focused / top-most. call
                           // before Begin()
...
void SetWindowFocus();   // (not recommended) set current window to be focused /
...
void SetWindowFontScale(
    float scale); // [OBSOLETE] set font scale. Adjust IO.FontGlobalScale if you
                  ...
                  // style.ScaleAllSizes().
void SetWindowPos(const char *name, const ImVec2 &pos,
...
void SetWindowFocus(const char *name);       // set named window to be focused /
...
ImVec2 GetContentRegionMax();   // current content boundaries (typically window
...
float GetScrollMaxX(); // get maximum scrolling amount ~~ ContentSize.x -
                       // WindowSize.x - DecorationsSize.x
float GetScrollMaxY(); // get maximum scrolling amount ~~ ContentSize.y -
...
ImGuiIO &GetIO(); // access the IO structure (mouse/keyboard/gamepad inputs,
...
ImDrawData *GetDrawData(); // valid after Render() and until the next call to
...
float GetTreeNodeToLabelSpacing(); // horizontal distance preceding label when
                                   // using TreeNode*() or Bullet() ==
                                   // (g.FontSize + style.FramePadding.x*2) for
                                   // a regular unframed TreeNode

bool CollapsingHeader(const char *label, ImTextureID *texture,
...
void BeginTooltip(); // begin/append a tooltip window. to create full-featured
                     // tooltip (with any kind of items).
void EndTooltip();
void SetTooltip(const char *fmt, ...) IM_FMTARGS(
...
void SetNextItemWidth(
    float item_width); // set width of the _next_ common large "item+label"
                       ...
                       // to the right of window (so -FLT_MIN always align width
                       // to the right side)
float CalcItemWidth(); // width of item given pushed settings and current cursor
...
bool Begin(const char *name, ImTextureID logo, bool *p_open = NULL,
...
bool InputFloat(const char *label, float *v, float step = 0.0f,
                ...
                ImGuiInputTextFlags flags = 0);
bool InputColorFloat(const char *label, float *v,
...
void SetItemDefaultFocus(); // make last item the default focused item of a
                            // window.
...
int GetKeyIndex(ImGuiKey key); // map ImGuiKey_* values into legacy native key
                               // index. == io.KeyMap[key]
#else
...
void EndChildFrame();            // always call EndChildFrame() regardless of
...
void EndDisabled();
void RenderArrow(ImDrawList *draw_list, ImTextureID *texture, ImVec2 pos,
                 ImU32 col, ImGuiDir dir, float scale);
void RenderArrow(ImDrawList *draw_list, ImVec2 pos, ImU32 col, ImGuiDir dir,
                 float scale = 1.0f);
void RenderBullet(ImDrawList *draw_list, ImTextureID *texture, ImVec2 pos,
                  ImU32 col);
...
ImDrawFlags CalcRoundingFlagsForRectInRect(const ImRect &r_in,
                                           ...
                                           float threshold);
int TableGetColumnCount(); // return number of columns (value passed to
                           // BeginTable)
...
void NextColumn();    // next column, defaults to current row or next row if the
                      // current row is finished
int GetColumnIndex(); // get current column index
float GetColumnWidth(int column_index = -1); // get column width (in pixels).
...
void TableHeadersRow(); // submit all headers cells based on data provided to
...
bool TableNextColumn(); // append into the next column (or first column of next
