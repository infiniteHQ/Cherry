...
#ifndef CHERRY_IMGUI_MARKDOWN
#define CHERRY_IMGUI_MARKDOWN
...
enum ImGuiMarkdownFormatFlags_ {
  ...
  ImGuiMarkdownFormatFlags_DiscardExtraNewLines =
      1 << 0, // (Accurate parsing) Provided markdown will discard all redundant
              // newlines
  ImGuiMarkdownFormatFlags_NoNewLineBeforeHeading =
      1 << 1, // (Accurate parsing) Provided markdown will not format a newline
              // after the first line if it is a heading
  ImGuiMarkdownFormatFlags_SeparatorDoesNotAdvance =
      1 << 2, // (Accurate parsing) Provided markdown will not advance to the
              // next line after formatting a separator
  ImGuiMarkdownFormatFlags_GithubStyle =
      ImGuiMarkdownFormatFlags_DiscardExtraNewLines |
      ImGuiMarkdownFormatFlags_NoNewLineBeforeHeading |
      ImGuiMarkdownFormatFlags_SeparatorDoesNotAdvance,
...
struct MarkdownImageData {
  ...
  bool useLinkCallback =
      false; // if true, linkCallback will be called when image is clicked
...
struct MarkdownFormatInfo {
  ...
  bool itemHovered = false; // Currently only set for links when mouse hovered,
                            // only valid when start_ == false
...
// - linkIcon is a string which encode a "Link" icon, if available in the
// current font (e.g. linkIcon = ICON_FA_LINK with FontAwesome +
// IconFontCppHeaders https://github.com/juliettef/IconFontCppHeaders)
// - headingFormats controls the format of heading H1 to H3, those above H3 use
// H3 format
struct MarkdownConfig {
  ...
  ImGuiMarkdownFormatFlags formatFlags =
      ImGuiMarkdownFormatFlags_None; // Configure this to change how Markdown
                                     // gets formatted. By default
                                     // imgui_markdown uses psuedo-Markdown for
...
inline void RenderLine(const char *markdown_, Line &line_,
...
struct TextRegion {
  ...
  bool RenderLinkText(const char *text_, const char *text_end_,
                      const Link &link_, const char *markdown_,
                      const MarkdownConfig &mdConfig_,
                      const char **linkHoverStart_);

  void RenderLinkTextWrapped(const char *text_, const char *text_end_,
                             const Link &link_, const char *markdown_,
                             const MarkdownConfig &mdConfig_,
                             const char **linkHoverStart_,
                             bool bIndentToHere_ = false);
...
// Text that starts after a new line (or at beginning) and ends with a newline
// (or at end)
struct Line {
...
inline void RenderLine(const char *markdown_, Line &line_,
  ...
  // indent
  int indentStart = 0;
  if (line_.isUnorderedListStart) // ImGui unordered list render always adds one
                                  // indent
  {
    indentStart = 1;
  }
  ...
  for (int i = 0; i < (int)markdownLength_; ++i) {
    ...
    if (line.isLeadingSpace) {
      if ((mdConfig_.formatFlags &
           ImGuiMarkdownFormatFlags_DiscardExtraNewLines)) // Discard LF and
                                                           // CRLF newlines by
                                                           // markdown spec
      {
        if (c == '\n') {
      ...
      if (c == ' ') {
        ...
        if ((c == '*') && (line.leadSpaceCount >= 2)) {
          ...
          if (line.isHeading) {
            // reset emphasis status, we do not support emphasis around headers
            // for now
            em = Emphasis();
            continue;
          }
    ...
    if ((mdConfig_.formatFlags &
         ImGuiMarkdownFormatFlags_DiscardExtraNewLines)) {
      // In markdown spec, 2 or more consecutive newlines gets converted to a
      // single blank line. The first newline is always digested by this parser
      // so we check for 1 or more here.
      if (!appliedExtraNewline && !prevLine.isHeading &&
          concurrentEmptyNewlines >= 1) {
        ImGui::NewLine();
    ...
    case Link::NO_LINK:
      if (c == '[' &&
          !line.isHeading) // we do not support headings with links for now
      {
    ...
    case Link::HAS_SQUARE_BRACKETS_ROUND_BRACKET_OPEN:
      ...
      if (link.num_brackets_open == 0) {
        // reset emphasis status, we do not support emphasis around links for
        // now
        em = Emphasis();
        ...
        link.url.stop = i;
        line.isUnorderedListStart =
            false; // the following text shouldn't have bullets
    ...
    case Emphasis::RIGHT:
      if (em.sym == c) {
        if (line.emphasisCount < 3 &&
            (i - em.text.stop + 1 == line.emphasisCount)) {
...
inline bool TextRegion::RenderLinkText(const char *text_, const char *text_end_,
  ...
  bool bHovered =
      bThisItemHovered || (*linkHoverStart_ == (markdown_ + link_.text.start));
  ...
  if (bHovered) {
    ...
    if (mdConfig_.tooltipCallback) {
      mdConfig_.tooltipCallback(
          {{markdown_ + link_.text.start, link_.text.size(),
            markdown_ + link_.url.start, link_.url.size(), mdConfig_.userData,
            false},
           mdConfig_.linkIcon});
    }
  }
...
inline void TextRegion::RenderTextWrapped(const char *text_,
  ...
  if (widthLeft > 0.0f) {
#if IMGUI_VERSION_NUM >= 19197
    endLine = ImGui::GetFont()->CalcWordWrapPosition(fontSize, text_, text_end_,
                                                     widthLeft);
#else
    endLine = ImGui::GetFont()->CalcWordWrapPositionA(scale, text_, text_end_,
                                                      widthLeft);
#endif
  }
  ...
  if (endLine > text_ && endLine < text_end_) {
    if (IsCharInsideWord(*endLine)) {
      ...
      float widthNextLine =
          ...
          GetWindowPos()
              .x; // was GetContentRegionMax().x on IMGUI_VERSION_NUM < 19099
#if IMGUI_VERSION_NUM >= 19197
...
inline void TextRegion::RenderLinkTextWrapped(
  ...
  bool bHovered = RenderLinkText(text_, endLine, link_, markdown_, mdConfig_,
                                 linkHoverStart_);
  if (bIndentToHere_) {
    float indentNeeded = GetContentRegionAvail().x - widthLeft;
    if (indentNeeded) {
  ...
  while (endLine < text_end_) {
    ...
    bool bThisLineHovered = RenderLinkText(text_, endLine, link_, markdown_,
                                           mdConfig_, linkHoverStart_);
...
defaultMarkdownFormatCallback(const MarkdownFormatInfo &markdownFormatInfo_,
  ...
  case MarkdownFormatType::EMPHASIS: {
    ...
    // default styling for emphasis uses last headingFormats - for your own
    // styling implement EMPHASIS in your formatCallback
...
#ifdef IMGUI_HAS_TEXTURES // used to detect dynamic font capability:
                          // https://github.com/ocornut/imgui/issues/8465#issuecomment-2701570771
        ImGui::PushFont(fmt.font, fmt.fontSize > 0.0f ? fmt.fontSize
                                                      : fmt.font->LegacySize);
#else
        ImGui::PushFont(fmt.font);
#endif
...
inline void MarkdownView(const std::string &raw) {
  mdConfig.linkCallback = NULL;
  mdConfig.tooltipCallback = NULL;
  mdConfig.imageCallback = NULL;
  mdConfig.linkIcon = NULL;

  H1 = Cherry::Application::GetFont("Bold");
  H2 = Cherry::Application::GetFont("Bold");
  H3 = Cherry::Application::GetFont("Bold");
  ...
  ImGui::Markdown(raw.c_str(), raw.length(), mdConfig);
}
} // namespace ImGui

#endif // CHERRY_IMGUI_MARKDOWN