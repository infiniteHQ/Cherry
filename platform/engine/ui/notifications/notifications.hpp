...
#define NOTIFY_RENDER_LIMIT                                                    \
  5 // Max number of toasts rendered at the same time. Set to 0 for unlimited
...
private:
  ...
  std::string color_bg = "#232323";
  std::string color_border = "#343434";
  std::string color_separator = "#343434";
...
public:
  inline void setBackgroundColor(const std::string &col) { color_bg = col; }

  inline void setBorderColor(const std::string &col) { color_border = col; }

  inline void setSeparatorColor(const std::string &col) {
    color_separator = col;
  }
...
public:
  ...
  inline std::string getBackgroundColor() { return this->color_bg; };

  inline std::string getBorderColor() { return this->color_border; };

  inline std::string getSeparatorColor() { return this->color_separator; };
...
public:
  ...
  ImGuiToast(ImGuiToastType type, int dismissTime = NOTIFY_DEFAULT_DISMISS,
             std::function<void()> actionBtnCallback = nullptr,
             std::function<bool()> btnCallback = nullptr,
             std::function<void()> renderCallback = nullptr,
             const ImTextureID &logo = nullptr) {
    IM_ASSERT(type < ImGuiToastType::COUNT);
...
inline void RenderNotifications() {
  ...
  for (size_t i = 0; i < notifications.size(); ++i) {
    ...
    ImGui::PushStyleColor(
        ImGuiCol_WindowBg,
        Cherry::HexToRGBA(currentToast->getBackgroundColor()));
    ImGui::PushStyleColor(ImGuiCol_Border,
                          Cherry::HexToRGBA(currentToast->getBorderColor()));
    ImGui::PushStyleColor(ImGuiCol_Separator,
                          Cherry::HexToRGBA(currentToast->getSeparatorColor()));
    ...
    // Here we render the toast content
    {
      ...
      if (NOTIFY_USE_DISMISS_BUTTON) {
        ...
        SetCursorPosX(GetCursorPosX() +
                      (GetWindowSize().x - GetCursorPosX()) * scale);
      ...
      if (wasTitleRendered && !NOTIFY_NULL_OR_EMPTY(content)) {
        SetCursorPosY(GetCursorPosY() +
                      25.f); // Must be a better way to do this!!!!
      }
