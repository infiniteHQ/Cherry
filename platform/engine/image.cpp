...
Image::Image(uint32_t width, uint32_t height, ImageFormat format,
             const std::string &winname, const void *data)
...
void Image::SetData(const void *data) {
  ...
  // Copy to Image
  {
    VkCommandBuffer command_buffer =
        Application::GetCommandBuffer(true, Cherry::GetWindowByName(m_Winname));
    if (command_buffer) {
...
void Image::SetData(const void *data, const std::string &winname) {
  ...
  {
    VkCommandBuffer command_buffer =
        Application::GetCommandBuffer(true, Cherry::GetWindowByName(winname));
