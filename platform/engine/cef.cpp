...
namespace Cherry
{
    ...
    VkCommandBuffer BeginSingleTimeCommands()
    {
        ...
        ImGui_ImplVulkanH_Window *wd = &Application::GetCurrentRenderedWindow()->m_WinData;
    ...
    ImTextureID CherryBrowser::GetCefTexture()
    {
        return cefTextureId;
    }
        ...
        if (type == PET_VIEW)
        {
            for (auto &browser : registered_browsers)
            {
                if (browser)
                {
                    if (browser->IsReady())
                    {
                        std::cout << "w : " << browser->m_Width << " /  " << width << std::endl;
                        std::cout << "h : " << browser->m_Height << " /  " << height << std::endl;
                        browser->UpdateCefTexture(buffer, browser->m_Width, browser->m_Height);
                    }
                    else
                    {
                        std::cout << "[WakeUp] Browser not ready, trying to wake it up." << std::endl;
                        browser->WakeUp();
                    }
                }
            }
    ...
    void CherryBrowser::WakeUp()
    {
        ...
        if (std::chrono::duration_cast<std::chrono::milliseconds>(now - lastWakeTime).count() < 500)
            return;
    ...
    void CherryBrowser::UpdateCefTexture(const void *buffer, int width, int height)
    {
        VkDevice device = Application::GetDevice();
        vkDeviceWaitIdle(device);

        if (textureImage == VK_NULL_HANDLE || width != m_Width || height != m_Height)
        {
    ...
    void CherryBrowser::ForwardImGuiInputsToCEF(CefRefPtr<CefBrowser> browser, const ImGuiIO &io, int browserWidth, int browserHeight)
    {
        if (!browser)
            return;
        ...
        for (int i = 0; i < 3; ++i)
        {
            if (io.MouseDown[i] != mouseDownPrev[i])
            {
                ...
                browser->GetHost()->SendMouseClickEvent(mouse_event, btn, !io.MouseDown[i], 1);
        ...
        if (io.MouseWheel != 0.0f || io.MouseWheelH != 0.0f)
        {
            browser->GetHost()->SendMouseWheelEvent(mouse_event,
                                                    static_cast<int>(io.MouseWheelH * 100),
    ...
    void CherryBrowser::ShowBrowserWindow(bool *p_open, ImTextureID tex_id)
    {
        ...
        if (tex_id != nullptr)
        {
            ...
            {
                ImGui::ImageButton(my_tex_id, ImVec2(m_Width, m_Height), ImVec2(0, 0), ImVec2(1, 1), ImColor(255, 255, 255, 255), ImColor(255, 255, 255, 128));
        ...
        else
        {
            ...
            {
                ImVec2 button_pos(
                    pos.x + (size.x - button_width) * 0.5f,
    ...
    void CherryBrowser::ProcessSDLEvent(const SDL_Event &event)
    {
        ...
        case SDL_KEYUP:
        {
            ...
            keyEvent.type = (event.type == SDL_KEYDOWN) ? KEYEVENT_RAWKEYDOWN : KEYEVENT_KEYUP;
        ...
        case SDL_MOUSEBUTTONUP:
        {
            ...
            browserHost->SendMouseClickEvent(mouseEvent, btnType, (event.type == SDL_MOUSEBUTTONUP), 1);
            break;
        }
        ...
        case SDL_MOUSEWHEEL:
        {
            ...
            browserHost->SendMouseWheelEvent(mouseEvent, event.wheel.x * 10, event.wheel.y * 10);
            break;
        }
        }
    }
    ...
    void CherryBrowser::InitCEF()
    {
        ...
        m_Browser = CefBrowserHost::CreateBrowserSync(window_info, m_BrowserClient, m_Url, settings, nullptr, nullptr);
    ...
    void BrowserView(const std::string &id, int width, int height, const std::string &url)
    {
        for (auto browser : registered_browsers)
        {
            if (browser->m_Id == id)
            {
                ...
                browser->ShowBrowserWindow(&browser->m_ShowWindow, browser->GetCefTexture());
                return;
            }
        }
