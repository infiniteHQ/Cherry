...
AppWindow::AppWindow(const std::string &id, const std::string &name)
    : m_ID(id), m_Name(name) {
  //
}
...
AppWindow::AppWindow(const std::string &id, const std::string &name,
    ...
    : m_ID(id), m_Name(name), m_Icon(icon) {
  SetIcon(m_Icon);
}
...
void AppWindow::CtxRender(std::vector<std::shared_ptr<RedockRequest>> *reqs,
  ...
  m_DockSpaceID = ImGui::GetID(m_HaveParentAppWindow ? "AppWindowDockspace"
                                                     : "MainDockspace");
  ...
  if (!ImGui::DockBuilderGetNode(m_DockSpaceID)) {
    ...
    ImGui::DockBuilderSetNodeSize(m_DockSpaceID,
  ...
  if (reqs) {
    for (auto it = reqs->begin(); it != reqs->end();) {
      ...
      ImGuiWindow *parentWindow =
          ImGui::FindWindowByName(req->m_ParentAppWindow.c_str());
      ...
      case DockEmplacement::DockUp:
        m_DockSpaceID = ImGui::DockBuilderSplitNode(
            parentDockID, ImGuiDir_Up, 0.5f, nullptr, &parentDockID);
        break;
      case DockEmplacement::DockDown:
        m_DockSpaceID = ImGui::DockBuilderSplitNode(
            parentDockID, ImGuiDir_Down, 0.5f, nullptr, &parentDockID);
        break;
      case DockEmplacement::DockLeft:
        m_DockSpaceID = ImGui::DockBuilderSplitNode(
            parentDockID, ImGuiDir_Left, 0.3f, nullptr, &parentDockID);
        break;
      case DockEmplacement::DockRight:
        m_DockSpaceID = ImGui::DockBuilderSplitNode(
            parentDockID, ImGuiDir_Right, 0.3f, nullptr, &parentDockID);
        break;
  ...
  if (m_Name.rfind("?loc:", 0) == 0) {
    ...
    window_name =
        Application::Get().GetLocale(localeName) + "####" + localeName;
  } else {
    window_name = m_Name;
  }
  ...
  if (wind->m_Specifications.RenderMode !=
      WindowRenderingMethod::SimpleWindow) {
    if (this->GetImage(m_Icon)) {
      ImTextureID texture =
          Application::Get().GetCurrentRenderedWindow()->get_texture(m_Icon);
      if (m_Closable) {
        ImGui::Begin(m_IdName.c_str(), texture, &m_CloseSignal, window_flags,
                     ImVec2(m_InternalPaddingX, m_InternalPaddingY));
      } else {
        ImGui::Begin(m_IdName.c_str(), texture, nullptr, window_flags,
                     ImVec2(m_InternalPaddingX, m_InternalPaddingY));
      }
    } else {
      if (m_Closable) {
    ...
    ImGui::BeginChild(ImGui::GetID("MainChild"), ImVec2(0, 0), false,
                      window_flags);
  }
  ...
  if (ImGui::BeginMenuBar()) {
    ...
    ImGui::GetFont()->Scale *=
        window_instance->m_Specifications.FontGlobalScale;
    ...
    float right_pos =
        ImGui::GetWindowWidth() - x_size - ImGui::GetStyle().ItemSpacing.x * 2;
  ...
  // Drag
  if (m_DockingMode || wind->m_Specifications.RenderMode ==
                           WindowRenderingMethod::DockingWindows) {
    if (ctx->DockTabStaticSelection.Pressed) {
      wind->drag_dropstate->DockIsDragging = true;
      wind->drag_dropstate->LastDraggingAppWindowHost =
          ctx->DockTabStaticSelection.TabName;
      Application::SetLastWindowPressed(
          wind->drag_dropstate->LastDraggingAppWindowHost);
    ...
    if (Application::GetCurrentDragDropState()) {
      if (Application::GetCurrentDragDropState()->DragOwner == this->m_IdName) {
        if (!ctx->DockTabStaticSelection.Pressed) {
          ...
          //{
          if (Application::GetCurrentDragDropState()->LastDraggingPlace ==
              DockEmplacement::DockBlank) {
            Application::GetCurrentDragDropState()->CreateNewWindow = true;
          } else {
            Application::PushRedockEvent(
    ...
    if (Application::GetDockIsDragging()) {
      if (m_DockingMode) {
        for (auto &win : Application::Get().m_AppWindows) {
          if (Application::GetCurrentDragDropState()
                  ->LastDraggingAppWindowHost == win->m_IdName) {
...
void AppWindow::SetSaved(const bool &new_state) { this->m_Saved = new_state; }

void AppWindow::SetOpened(const bool &new_state) { this->m_Opened = new_state; }
...
void AppWindow::SetSimpleStorage(const std::string &key,
                                 const std::string &data,
                                 const bool &persistant) {
  std::shared_ptr<SimpleStorageItem> value =
      std::make_shared<SimpleStorageItem>(data, persistant);
  m_Storage[key] = value;
}

void AppWindow::SetWindowStorage(const std::string &key,
                                 const nlohmann::json &data,
                                 const bool &persistant) {
  std::shared_ptr<WindowStorageItem> value =
      std::make_shared<WindowStorageItem>(data, persistant);
  m_WindowStorage[key] = value;
}

std::shared_ptr<SimpleStorageItem>
AppWindow::GetSimpleStorage(const std::string &key) {
...
std::shared_ptr<WindowStorageItem>
AppWindow::GetWindowStorage(const std::string &key) {
...
AppWindow::DumpWindowStorage() {
  return m_WindowStorage;
}
std::unordered_map<std::string, std::shared_ptr<SimpleStorageItem>>
AppWindow::DumpSimpleStorage() {
  return m_Storage;
}

void AppWindow::SetDefaultBehavior(DefaultAppWindowBehaviors behavior,
...
void AppWindow::SetLeftMenubarCallback(
    const std::function<void()> &right_menubar) {
  m_MenubarLeft = right_menubar;
  m_EnableMenuBar = true;
}

void AppWindow::SetRightMenubarCallback(
    const std::function<void()> &left_menubar) {
  m_MenubarRight = left_menubar;
  m_EnableMenuBar = true;
}

void AppWindow::SetLeftBottombarCallback(
    const std::function<void()> &left_menubar) {
  m_BottombarLeft = left_menubar;
  m_EnableBottomBar = true;
}

void AppWindow::SetRightBottombarCallback(
    const std::function<void()> &right_menubar) {
  m_BottombarRight = right_menubar;
  m_EnableBottomBar = true;
}

void AppWindow::SetIcon(const std::string &name) { m_Icon = name; }

void AppWindow::SetDockingMode(const bool &use_docking) {
  m_DockingMode = use_docking;
}

void AppWindow::SetFetchedSaveData(const std::string &key,
                                   const std::string &value) {
...
ImTextureID *AppWindow::GetTexture(const std::string &path) {
  for (auto &win : Application::Get().m_Windows) {
    if (this->CheckWinParent(win->GetName())) {
      static ImTextureID logoID = this->GetImage(m_Icon)->GetImGuiTextureID(
...
AppWindowWrapper::AppWindowWrapper() {
  m_AppWindow = std::make_shared<Cherry::AppWindow>("undefined_window",
                                                    "undefined_window");
}
...
std::shared_ptr<AppWindowWrapper>
AppWindowWrapper::Create(const std::string &name,
                         const std::function<void()> callback) {
  auto instance =
      std::shared_ptr<AppWindowWrapper>(new AppWindowWrapper(name, callback));
