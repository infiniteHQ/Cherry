...
// Audio service if CHERRY_ENABLE_AUDIO
  ...
  void Application::DestroyComponent(const Identifier &id, ComponentsPool *pool) {
    ...
    if (beforeSize == afterSize) {
      std::cout << "[LOG] Aucun composant avec l'identifiant " << id.string() << " n'a été trouvé pour suppression."
                << std::endl;
    } else {
      std::cout << "[LOG] Composant avec l'identifiant " << id.string() << " supprimé avec succès." << std::endl;
    }
