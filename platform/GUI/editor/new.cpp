...
#include "Embed/WindowImages.embed"
    ...
    void Application::Init()
    {
        ...
        //this->m_Windows.push_back(std::make_shared<Window>("2", app->m_Specification.Width - 500, app->m_Specification.Height - 500));
    }
    ...
    void MouseButtonCallback(GLFWwindow *window, int button, int action, int mods)
    {
        ...
        if (win)
        {
            std::shared_ptr<WindowClickEvent> pendingClick = std::make_shared<WindowClickEvent>();
            pendingClick->action = action;
            pendingClick->button = button;
            pendingClick->mods = mods;
            win->m_PendingClick = pendingClick;
        }
        else
        {
            //
    ...
    void MouseMoveCallback(GLFWwindow *window, double xpos, double ypos)
    {
        ...
        if (win)
        {
            std::shared_ptr<WindowMoveEvent> pendingMove = std::make_shared<WindowMoveEvent>();
            pendingMove->xpos = xpos;
            pendingMove->ypos = ypos;
            win->m_PendingMove = pendingMove;
        }
        else
        {
            //
        ...
        : m_Name(name), m_Width(width), m_Height(height)
    {
        ...
        glfwSetCursorPosCallback(this->GetWindowHandle(), MouseMoveCallback);
    ...
    ImDrawData *Application::RenderWindow(Window *window)
    {
        ...
        if (window->m_PendingMove != nullptr)
        {
            std::cout << "Existing move : " << io.MousePos.x << " / " << io.MousePos.y << std::endl;
            io.MousePos = ImVec2((float)window->m_PendingMove->xpos, (float)window->m_PendingMove->ypos);

            window->m_PendingMove = nullptr;
        }

        if (window->m_PendingClick != nullptr)
        {
            if (window->m_PendingClick->action)
            {
                std::cout << "Click detected -> " << window->GetName() << std::endl;
            }

            if (window->m_PendingClick->button >= 0 && window->m_PendingClick->button < IM_ARRAYSIZE(io.MouseDown))
                io.MouseDown[window->m_PendingClick->button] = (window->m_PendingClick->action == GLFW_PRESS);
        }
        ...
        glfwGetCursorPos(window->GetWindowHandle(), &cxpos, &cypos);
        ...
        ImGui::SetNextWindowPos(ImVec2(window->m_PosX, window->m_PosY));
        ImGui::SetNextWindowSize(ImVec2(window->m_Width, window->m_Height));
        ImGui::SetNextWindowViewport(viewport->ID);
        ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, 0.0f);
        ImGui::PushStyleVar(ImGuiStyleVar_WindowBorderSize, 0.0f);

        std::cout << "Viewport m_PosX " << xpos << std::endl;
        std::cout << "Viewport m_PosY " << ypos << std::endl;
        std::cout << "Viewport m_Width " << window->m_Width << std::endl;
        std::cout << "Viewport m_Height " << window->m_Height << std::endl;
        ...
        static std::vector<ImGuiWindow *> wins;
        ...
        for (auto &layer : m_LayerStack)
        {
            ...
            layer->m_WindowControlCallbalck = [window, io, viewport, xpos, ypos, width, height](ImGuiWindow *win)
            {
                wins.push_back(win);
            };
        ...
        if (DEBUG_WINDOW)
        {
            ...
            std::cout << std::endl;
            std::cout << "┌──────────────────────────────────────────────────────" << std::endl;
            std::cout << "│  Vortex Window \"" + name + "\"" << std::endl;
            std::cout << "├──────────────────────────────────────────────────────" << std::endl;
            std::cout << "│ MousePos X        │ " << io.MousePos.x << std::endl;
            std::cout << "│ MousePos Y        │ " << io.MousePos.y << std::endl;
            std::cout << "│ MousePos GLFW X   │ " << cxpos << std::endl;
            std::cout << "│ MousePos GLFW Y   │ " << cypos << std::endl;
            std::cout << "│ MousePos Left     │ " << io.MouseDown[0] << std::endl;
            std::cout << "│ MousePos Right    │ " << io.MouseDown[1] << std::endl;
            std::cout << "│ ImGui MouseVP PosX│ " << m_ImGuiContext->MouseViewport->Pos.x << std::endl;
            std::cout << "│ ImGui MouseVP PosY│ " << m_ImGuiContext->MouseViewport->Pos.y << std::endl;
            std::cout << "│ GLFW X            │ " << xpos << std::endl;
            std::cout << "│ GLFW Y            │ " << ypos << std::endl;
            std::cout << "│ GLFW Size X       │ " << width << std::endl;
            std::cout << "│ GLFW Size Y       │ " << height << std::endl;
            std::cout << "│ Viewport X        │ " << viewport->Pos.x << std::endl;
            std::cout << "│ Viewport Y        │ " << viewport->Pos.y << std::endl;
            std::cout << "│ Viewport Size X   │ " << viewport->Size.x << std::endl;
            std::cout << "│ Viewport Size Y   │ " << viewport->Size.y << std::endl;
            std::cout << "│ io.DisplaySize X  │ " << io.DisplaySize.x << std::endl;
            std::cout << "│ io.DisplaySize Y  │ " << io.DisplaySize.y << std::endl;
            ...
            /*for (auto win : wins)
            {
                if (win)
                {
                    ...
                    std::cout << "│ Dock Node         │ " << win->DockNode << std::endl;
                    std::cout << "│ Dock Tree Name    │ " << win->RootWindowDockTree->Name << std::endl;
                    std::string str = win->RootWindowDockTree->Name;
                    std::string str_finded = str.substr(str.find(".") + 1, str.size());
                    std::cout << "│ Deducted tree name│ " << str_finded << std::endl;
                    ...
                    {
                        std::cout << "│ Window alone      │ " << std::endl;
                    }
                    std::cout << "│ Dock ID           │ " << win->DockId << std::endl;
                    std::cout << "│ Dock Host ID      │ " << win->DockNodeAsHost << std::endl;
                    ...
                    {
                        std::cout << "│ Moved             │ True" << std::endl;
                    }
                    else
                    {
                        std::cout << "│ Moved             │ False" << std::endl;
                    }
                    std::cout << "└───────────────────┘──────────────────────────────────" << std::endl;
                }
                else
        ...
        if (window->m_IsDragging && !window->m_ResizePending)
        {
            if (ImGui::IsMouseDown(ImGuiMouseButton_Left))
            {
                ...
                { // Only update position if there is a change
                    ...
                    window->m_PosX = (int)newWindowPos.x;
                    window->m_PosY = (int)newWindowPos.y;
