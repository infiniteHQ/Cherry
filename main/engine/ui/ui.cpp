// OBSOLETE
// Will be mouved to main/engine/drawing
...
namespace Cherry::UI {
  ...
  void DrawButtonImage(
      const std::shared_ptr<Cherry::Image>& imageNormal,
      const std::shared_ptr<Cherry::Image>& imageHovered,
      const std::shared_ptr<Cherry::Image>& imagePressed,
      ImU32 tintNormal,
      ImU32 tintHovered,
      ImU32 tintPressed,
      ImVec2 rectMin,
  ...
  void DrawButtonImage(
      ...
      ImRect rectangle) {
    DrawButtonImage(
        imageNormal, imageHovered, imagePressed, tintNormal, tintHovered, tintPressed, rectangle.Min, rectangle.Max);
  };

  void DrawButtonImage(
      const std::shared_ptr<Cherry::Image>& image,
      ImU32 tintNormal,
      ImU32 tintHovered,
      ImU32 tintPressed,
      ImVec2 rectMin,
  ...
  void DrawButtonImage(const std::shared_ptr<Cherry::Image>& image, ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed) {
    DrawButtonImage(
        image, image, image, tintNormal, tintHovered, tintPressed, ImGui::GetItemRectMin(), ImGui::GetItemRectMax());
  };
  ...
  bool UpdateWindowManualResize(ImGuiWindow* window, ImVec2& newSize, ImVec2& newPosition) {
    ...
    // Clip mouse interaction rectangles within the viewport rectangle (in practice the narrowing is going to happen most of
    // the time).
    ...
    // window have such threshold). We only clip interaction so we overwrite window->ClipRect, cannot call PushClipRect() yet
    // as DrawList is not yet setup.
  ...
  bool BeginMenubar(const ImRect& barRectangle) {
    ...
    ImRect clip_rect(
        ...
        IM_ROUND(
            ImMax(bar_rect.Min.x + window->Pos.x, bar_rect.Max.x - ImMax(window->WindowRounding, window->WindowBorderSize))),
  ...
  void DrawBorder(ImRect rect, float thickness, float rounding, float offsetX, float offsetY) {
    ...
    drawList->AddRect(
        min, max, ImGui::ColorConvertFloat4ToU32(ImGui::GetStyleColorVec4(ImGuiCol_Border)), rounding, 0, thickness);
  };
