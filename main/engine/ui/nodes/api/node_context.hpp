...
public:
  explicit NodeContext(const std::string &contextID = "")
      : m_ContextID(contextID), m_AllowComments(false),
        m_AllowSequences(false) {}
  ...
  std::vector<NodeSchema *> GetSchemas() const {
    std::vector<NodeSchema *> result;
    result.reserve(m_SchemaMap.size());
    for (const auto &kv : m_SchemaMap)
