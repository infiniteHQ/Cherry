//
//  console.cpp
//  Sources for devtools console/prompt
//
...
void ConsoleView::Render() {
  ...
  for (const auto &e : s_Entries) {
    if (e.level == Cherry::Log::Level::Error || e.level == Cherry::Log::Level::Fatal)
      errors++;
  ...
  ImVec2 center_err = ImVec2(p_err.x + icon_size * 0.5f, p_err.y + icon_size * 0.5f);
  draw_list->AddCircleFilled(center_err, icon_size * 0.45f, IM_COL32(255, 80, 80, 255));
  draw_list->AddLine(
      ImVec2(center_err.x - 3, center_err.y - 3), ImVec2(center_err.x + 3, center_err.y + 3), IM_COL32_WHITE, 1.5f);
  draw_list->AddLine(
      ImVec2(center_err.x + 3, center_err.y - 3), ImVec2(center_err.x - 3, center_err.y + 3), IM_COL32_WHITE, 1.5f);
  ...
  ImVec2 center_warn = ImVec2(p_warn.x + icon_size * 0.5f, p_warn.y + icon_size * 0.5f);
  ImVec2 t1(center_warn.x, center_warn.y - 6), t2(center_warn.x - 7, center_warn.y + 6),
  ...
  ImGui::Separator();
  const float footer_height = ImGui::GetFrameHeightWithSpacing() + ImGui::GetStyle().ItemSpacing.y + 12.0f;
  ...
  for (int i = 0; i < (int)s_Entries.size(); i++) {
    ...
    ImVec4 bg_color = (i % 2 == 0) ? ImVec4(1, 1, 1, 0.02f) : ImVec4(0, 0, 0, 0);
    ...
    draw_list->AddRectFilled(
        pos, ImVec2(pos.x + ImGui::GetContentRegionAvail().x, pos.y + line_h), ImGui::ColorConvertFloat4ToU32(bg_color));
    ...
    if (entry.level == Cherry::Log::Level::Fatal) {
      ...
      draw_list->AddLine(
          ImVec2(icon_pos.x - 3, icon_pos.y - 3), ImVec2(icon_pos.x + 3, icon_pos.y + 3), IM_COL32_WHITE, 1.6f);
      draw_list->AddLine(
          ImVec2(icon_pos.x + 3, icon_pos.y - 3), ImVec2(icon_pos.x - 3, icon_pos.y + 3), IM_COL32_WHITE, 1.6f);
    } else if (entry.level == Cherry::Log::Level::Warn) {
      ImVec2 p1(icon_pos.x, icon_pos.y - 7), p2(icon_pos.x - 7, icon_pos.y + 6), p3(icon_pos.x + 7, icon_pos.y + 6);
      draw_list->AddTriangleFilled(p1, p2, p3, icon_color);
    } else {
      draw_list->AddCircle(icon_pos, 5.0f, IM_COL32(150, 150, 150, 120), 12, 1.0f);
    }
  ...
  if (!matches.empty()) {
    ImVec2 pos = ImGui::GetCursorScreenPos();
    float line_h = ImGui::GetTextLineHeightWithSpacing();
    ImGui::SetNextWindowPos(ImVec2(pos.x + 40, pos.y - (line_h * (float)matches.size()) - 20));
    ...
    if (ImGui::Begin(
      ...
      for (int i = 0; i < (int)matches.size(); i++) {
        ...
        ImGui::TextColored(ImVec4(0.4f, 0.75f, 1.0f, 1.0f), "%s", matches[i]->name.c_str());
        ImGui::SameLine();
        ImGui::TextDisabled("(");
        ImGui::SameLine();
        ImGui::TextColored(ImVec4(0.85f, 0.55f, 0.3f, 1.0f), "%s", matches[i]->params.c_str());
        ImGui::SameLine();
        ImGui::TextDisabled(")");
  ...
  draw_list->AddRect(
      ImVec2(input_cursor_pos.x + 6, input_cursor_pos.y + 4),
      ImVec2(input_cursor_pos.x + 26, input_cursor_pos.y + 20),
      term_color,
      1.0f);
  draw_list->AddLine(
      ImVec2(input_cursor_pos.x + 10, input_cursor_pos.y + 9),
      ImVec2(input_cursor_pos.x + 13, input_cursor_pos.y + 12),
      term_color,
      1.2f);
  draw_list->AddLine(
      ImVec2(input_cursor_pos.x + 10, input_cursor_pos.y + 15),
      ImVec2(input_cursor_pos.x + 13, input_cursor_pos.y + 12),
      term_color,
      1.2f);
  draw_list->AddLine(
      ImVec2(input_cursor_pos.x + 15, input_cursor_pos.y + 16),
      ...
      case ImGuiInputTextFlags_CallbackHistory: {
        if (input.empty()) {
          ...
          if (data->EventKey == ImGuiKey_UpArrow) {
            ...
            if (s_HistoryPos != -1 && ++s_HistoryPos >= (int)s_CommandHistory.size())
              s_HistoryPos = -1;
          }
  ...
  if (ImGui::InputTextWithHint(
          "##ConsoleInput", "Execute Lua command...", s_InputBuffer, IM_ARRAYSIZE(s_InputBuffer), input_flags, callback)) {
...
void ConsoleView::AddLogEntry(Cherry::Log::Level level, const std::string &msg) {
