...
namespace ImGui {
  // Docking
  static const float DOCKING_TRANSPARENT_PAYLOAD_ALPHA = 0.50f;  // For use with io.ConfigDockingTransparentPayload. Apply to
  ...
  void DockNodePreviewDockR(
    ...
    if (data->IsDropAllowed && can_preview_tabs && data->SplitDir == ImGuiDir_None && data->IsCenterAvailable) {
      ...
      if (host_node && host_node->TabBar) {
        if (!host_node->IsHiddenTabBar() && !host_node->IsNoTabBar())
          tab_pos.x +=
              host_node->TabBar->WidthAllTabs + g.Style.ItemInnerSpacing.x;  // We don't use OffsetNewTab because when using
                                                                             // non-persistent-order tab bar it is
                                                                             // incremented with each Tab submission.
        else
        ...
        tab_pos.x += g.Style.ItemInnerSpacing.x + TabItemCalcSize(host_window->Name, host_window->HasCloseButton)
    ...
    for (int dir = ImGuiDir_None; dir < ImGuiDir_COUNT; dir++) {
      ...
      if (dir == ImGuiDir_None && (host_node && (host_node->MergedFlags & ImGuiDockNodeFlags_NoSplit)) ||
...
namespace Cherry {
  ...
  Window::Window(const std::string &name, int width, int height, ApplicationSpecification specs, bool cold_start)
    ...
    if (m_Specifications.DisableWindowManagerTitleBar) {
      window_flags =
          SDL_WindowFlags(SDL_WINDOW_VULKAN | SDL_WINDOW_RESIZABLE | SDL_WINDOW_ALLOW_HIGHDPI | SDL_WINDOW_BORDERLESS);
    } else {
    ...
    m_WindowHandler =
        SDL_CreateWindow(m_Name.c_str(), SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, m_Width, m_Height, window_flags);
...
#ifdef CHERRY_ENABLE_LINUXDRM
    if (!SDL_Vulkan_CreateSurface(m_WindowHandler, Application::GetInstance(), &m_Surface)) {
      printf(
          "Warning: SDL Vulkan surface creation failed on DRM. "
...

    {
      ...
      check_vk_result(err);
      err = vkQueueSubmit(Application::GetVkQueue(), 1, &end_info, VK_NULL_HANDLE);
      check_vk_result(err);
    ...
    auto ShowDropZones = [&](ImGuiDockNode *node) {
      ...
      for (int dir = ImGuiDir_None; dir < ImGuiDir_COUNT; dir++) {
        ...
        if (drop_rect.Contains(
                ImVec2(Application::GetCurrentDragDropState()->mouseX, Application::GetCurrentDragDropState()->mouseY)) &&
          ...
          if (dir == ImGuiDir_None) {
            ...
            switch (dir) {
              ...
              case ImGuiDir_Down:
                ...
                break;
              default: Application::SetCurrentDragDropStateDraggingPlace(Cherry::DockEmplacement::DockBlank); break;
            }
          }
    ...
    if (m_Specifications.CustomTitlebar) {
      {
        ...
        if (!m_Specifications.WindowOnlyClosable) {
          ...
          UI::DrawButtonImage(
              this->get(g_WindowMinimizeIcon, "Minimize"),
              buttonColN,
              buttonColH,
              buttonColP,
      ...
      }
      {
        ...
        UI::ShiftCursorY(8.0f);
        {
          ...
          UI::DrawButtonImage(
              this->get(g_WindowCloseIcon, "Close"),
      ...
      if (m_ClosePending) {
        if (m_Specifications.UsingCloseCallback) {
          ...
          if (Application::Get().GetCloseCallback()) {
            Application::Get().GetCloseCallback()();
          } else {
    ...
    if (m_Specifications.CustomTitlebar) {
      ...
      if (UI::BeginMenubar(menuBarRect)) {
        ...
        if (this->m_Specifications.MenubarCallback) {
          ...
          Application::Get().GetMenubarCallback()();
        }
  ...
  std::shared_ptr<Cherry::Image> Window::add(const std::string &path) {
    ...
    std::shared_ptr<Cherry::Image> _icon =
        std::make_shared<Cherry::Image>(w, h, Cherry::ImageFormat::RGBA, this->GetName(), data);
  ...
  std::shared_ptr<Cherry::Image> Window::add(const uint8_t data[], const std::string &name) {
    ...
    std::shared_ptr<Cherry::Image> _icon =
        std::make_shared<Cherry::Image>(w, h, Cherry::ImageFormat::RGBA, this->GetName(), icondata);
  ...
  void Window::LoadTheme() {
    ImGui::PushStyleColor(
        ImGuiCol_Header,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_header")));  // color_header
    ImGui::PushStyleColor(
        ImGuiCol_HeaderHovered,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_header_hovered")));  // color_header_hovered
    ImGui::PushStyleColor(
        ImGuiCol_HeaderActive,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_header_active")));  // color_header_active

    ImGui::PushStyleColor(
        ImGuiCol_Button,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("button_color_border")));  // color_button
    ImGui::PushStyleColor(
        ImGuiCol_ButtonHovered,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("button_color_border_hovered")));  // color_button_hovered
    ImGui::PushStyleColor(
        ImGuiCol_ButtonActive,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("button_color_border_pressed")));  // color_button_active

    ImGui::PushStyleColor(
        ImGuiCol_FrameBg,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_framebg")));  // color_framebg
    ImGui::PushStyleColor(
        ImGuiCol_FrameBgHovered,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_framebg_hovered")));  // color_framebg_hovered
    ImGui::PushStyleColor(
        ImGuiCol_FrameBgActive,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_framebg_active")));  // color_framebg_active

    ImGui::PushStyleColor(ImGuiCol_Text, Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_text")));

    ImGui::PushStyleColor(ImGuiCol_Tab,
                          Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_tab")));  // color_tab
    ImGui::PushStyleColor(
        ImGuiCol_TabHovered,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_tab_hovered")));  // color_tab_hovered
    ImGui::PushStyleColor(
        ImGuiCol_TabActive,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_tab_active")));  // color_tab_active
    ImGui::PushStyleColor(
        ImGuiCol_TabUnfocused,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_tab_unfocused")));  // color_tab_unfocused
    ImGui::PushStyleColor(
        ImGuiCol_TabUnfocusedActive,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_tab_unfocused_active")));  // color_tab_unfocused_active

    ImGui::PushStyleColor(
        ImGuiCol_TitleBg,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_titlebg")));  // color_titlebg
    ImGui::PushStyleColor(
        ImGuiCol_TitleBgActive,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_titlebg_active")));  // color_titlebg_active
    ImGui::PushStyleColor(
        ImGuiCol_TitleBgCollapsed,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_titlebg_collapsed")));  // color_titlebg_collapsed

    ImGui::PushStyleColor(
        ImGuiCol_ResizeGrip,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_resizegrip")));  // color_resizegrip
    ImGui::PushStyleColor(
        ImGuiCol_ResizeGripHovered,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_resizegrip_hovered")));  // color_resizegrip_hovered
    ImGui::PushStyleColor(
        ImGuiCol_ResizeGripActive,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_resizegrip_active")));  // color_resizegrip_active

    ImGui::PushStyleColor(
        ImGuiCol_ScrollbarBg,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_scrollbar_bg")));  // color_scrollbar_bg

    ImGui::PushStyleColor(
        ImGuiCol_ScrollbarGrab,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_scrollbar_grab")));  // color_scrollbar_grab
    ImGui::PushStyleColor(
        ImGuiCol_ScrollbarGrabHovered,
        Cherry::HexToRGBA(
            CherryApp.GetActiveThemeProperty("color_scrollbar_grab_hovered")));  // color_scrollbar_grab_hovered
    ImGui::PushStyleColor(
        ImGuiCol_ScrollbarGrabActive,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_scrollbar_grab_active")));  // color_scrollbar_grab_active

    ImGui::PushStyleColor(
        ImGuiCol_CheckMark,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_checkmark")));  // color_checkmark

    ImGui::PushStyleColor(
        ImGuiCol_SliderGrab,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_slidergrab")));  // color_slidergrab
    ImGui::PushStyleColor(
        ImGuiCol_SliderGrabActive,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_slidergrab_active")));  // color_slidergrab_active

    ImGui::PushStyleColor(
        ImGuiCol_Separator,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_separator")));  // color_separator

    ImGui::PushStyleColor(
        ImGuiCol_SeparatorActive,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_separator_active")));  // color_separator_active
    ImGui::PushStyleColor(
        ImGuiCol_SeparatorHovered,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_separator_hovered")));  // color_separator_hovered

    ImGui::PushStyleColor(
        ImGuiCol_WindowBg,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_window_bg")));  // color_window_bg
    ImGui::PushStyleColor(
        ImGuiCol_ChildBg,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_child_bg")));  // color_child_bg
    ImGui::PushStyleColor(
        ImGuiCol_PopupBg,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_popup_bg")));  // color_popup_bg
    ImGui::PushStyleColor(
        ImGuiCol_Border,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_border")));  // color_border

    ImGui::PushStyleColor(
        ImGuiCol_TableHeaderBg,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_table_header_bg")));  // color_table_header_bg
    ImGui::PushStyleColor(
        ImGuiCol_TableBorderLight,
        Cherry::HexToRGBA(CherryApp.GetActiveThemeProperty("color_table_border_light")));  // color_table_border_light
  ...
  void Window::UnloadTheme() {
    ImGui::PopStyleColor(38);
  }

  void Window::CleanupVulkanWindow() {
    ImGui_ImplVulkanH_DestroyWindow(
        Application::GetInstance(), Application::GetDevice(), &m_WinData, &Application::GetVkAllocationCallbacks());
  }
