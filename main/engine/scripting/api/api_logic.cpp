//
//  api_logic.cpp
//  Main scripting API in lua for all logic features of Cherry
//
...
namespace Cherry {
  namespace Script {
    ...
    static Cherry::Component &Lua_GetComponentHelper(lua_State *L, const std::string &id_str) {
    ...
    LUA_FUNC(IsMouseClickedOnPos) {
      ...
      bool inside =
          (mousePos.x >= winPos.x + x && mousePos.x <= winPos.x + x + w && mousePos.y >= winPos.y + y &&
           mousePos.y <= winPos.y + y + h);
    ...
    LUA_FUNC(IsMouseClickedOnCurrentPos) {
      ...
      bool inside = mousePos.x >= winPos.x + x && mousePos.x <= winPos.x + x + w && mousePos.y >= winPos.y + y &&
                    mousePos.y <= winPos.y + y + h;

      bool windowHovered = ImGui::IsWindowHovered(ImGuiHoveredFlags_RootAndChildWindows);

      bool clicked = inside && windowHovered && CherryApp.IsMouseClicked((ImGuiMouseButton)btn, repeat);
    ...
    LUA_FUNC(BeginComponent) {
      ...
      auto component = CherryApp.PushComponent<Cherry::Component>(Cherry::Identifier(id, Cherry::RenderMode::CreateOnly));
