...
namespace Cherry {
  namespace GUI {
    ...
    CalcWordWrapPositionA(ImFont *ptr, float scale, const char *text, const char *text_end, float wrap_width);
    CHERRY_API void RenderChar(ImFont *ptr, ImDrawList *draw_list, float size, ImVec2 pos, ImU32 col, ImWchar c);
    CHERRY_API void RenderText(
        ImFont *ptr,
    ...
    CHERRY_API void AddRectFilledMultiColor(
        ImDrawList *ptr,
        const ImVec2 &p_min,
        const ImVec2 &p_max,
        ImU32 col_upr_left,
    ...
    CHERRY_API void
    AddQuadFilled(ImDrawList *ptr, const ImVec2 &p1, const ImVec2 &p2, const ImVec2 &p3, const ImVec2 &p4, ImU32 col);
    CHERRY_API void
    AddTriangle(ImDrawList *ptr, const ImVec2 &p1, const ImVec2 &p2, const ImVec2 &p3, ImU32 col, float thickness = 1.0f);
    CHERRY_API void AddTriangleFilled(ImDrawList *ptr, const ImVec2 &p1, const ImVec2 &p2, const ImVec2 &p3, ImU32 col);
    CHERRY_API void
    AddCircle(ImDrawList *ptr, const ImVec2 &center, float radius, ImU32 col, int num_segments = 0, float thickness = 1.0f);
    CHERRY_API void AddCircleFilled(ImDrawList *ptr, const ImVec2 &center, float radius, ImU32 col, int num_segments = 0);
    CHERRY_API void
    AddNgon(ImDrawList *ptr, const ImVec2 &center, float radius, ImU32 col, int num_segments, float thickness = 1.0f);
    CHERRY_API void AddNgonFilled(ImDrawList *ptr, const ImVec2 &center, float radius, ImU32 col, int num_segments);
    CHERRY_API void
    AddText(ImDrawList *ptr, const ImVec2 &pos, ImU32 col, const char *text_begin, const char *text_end = NULL);
    CHERRY_API void AddText(
        ImDrawList *ptr,
        const ImFont *font,
        float font_size,
        const ImVec2 &pos,
    ...
    CHERRY_API void
    AddPolyline(ImDrawList *ptr, const ImVec2 *points, int num_points, ImU32 col, ImDrawFlags flags, float thickness);

    CHERRY_API void PushClipRect(
        ImDrawList *ptr,
    ...
    CHERRY_API void AddConvexPolyFilled(
        ImDrawList *ptr,
        const ImVec2 *points,
        int num_points,
        ImU32 col);  // Note: Anti-aliased filling requires points to
                     // be in clockwise order.
    CHERRY_API void AddBezierCubic(
        ImDrawList *ptr,
        const ImVec2 &p1,
        const ImVec2 &p2,
        const ImVec2 &p3,
    ...
    CHERRY_API void AddBezierQuadratic(
        ImDrawList *ptr,
        const ImVec2 &p1,
        const ImVec2 &p2,
        const ImVec2 &p3,
    ...
    CHERRY_API bool BeginMenuBar(float offsetY);  // append to menu-bar of current window (requires
                                                  // ImGuiWindowFlags_MenuBar flag set on parent window).
    ...
    CHERRY_API bool ImageSizeButtonWithText(
        ImTextureID texId,
        float button_width,
        const char *label,
    ...
    CHERRY_API bool CheckboxFlags(const char *label, unsigned int *flags, unsigned int flags_value);
    CHERRY_API bool RadioButton(
        const char *label,
    ...
    CHERRY_API void ProgressBar(float fraction, const ImVec2 &size_arg = ImVec2(-FLT_MIN, 0), const char *overlay = NULL);
    ...
    CHERRY_API bool TextButtonWithImageWithIcon(
        ...
        const ImVec4 &bg_col = ImVec4(0, 0, 0, 0),
        const ImVec4 &tint_col = ImVec4(1, 1, 1, 1));
    CHERRY_API bool ImageButtonWithTextWithIcon(
        ImTextureID texIdFirst,
        ImTextureID texIdSecond,
        const char *label,
        const ImVec2 &imageSizeFirst = ImVec2(0, 0),
        ...
        const ImVec4 &bg_col = ImVec4(0, 0, 0, 0),
        const ImVec4 &tint_col = ImVec4(1, 1, 1, 1));
    CHERRY_API bool ImageButtonWithText(
        ImTextureID texId,
        const char *label,
        const ImVec2 &imageSize = ImVec2(0, 0),
        const ImVec2 &uv0 = ImVec2(0, 0),
        ...
        const ImVec4 &bg_col = ImVec4(0, 0, 0, 0),
        const ImVec4 &tint_col = ImVec4(1, 1, 1, 1));
    CHERRY_API bool RightImageButtonWithText(
        ImTextureID texId,
        const char *label,
        const ImVec2 &imageSize = ImVec2(0, 0),
        const ImVec2 &uv0 = ImVec2(0, 0),
    ...
    CHERRY_API bool BeginPopupContextItem(
        const char *str_id = NULL,
        ImGuiPopupFlags popup_flags = 1);  // open+begin popup when clicked on last item. Use str_id==NULL to
    ...
    CHERRY_API float GetFontSize();              // get current font size (= height in pixels) of
    ...
    CHERRY_API ImU32 GetColorU32(const ImVec4 &col);           // retrieve given color with style alpha applied, packed
                                                               // as a 32-bit value suitable for ImDrawList
    CHERRY_API ImU32 GetColorU32(ImU32 col);                   // retrieve given color with style alpha applied, packed
                                                               // as a 32-bit value suitable for ImDrawList
    CHERRY_API const ImVec4 &GetStyleColorVec4(ImGuiCol idx);  // retrieve style color as stored in ImGuiStyle structure. use
    ...
    CHERRY_API void TextColored(const ImVec4 &col, const char *fmt, ...)
        IM_FMTARGS(2);  // shortcut for PushStyleColor(ImGuiCol_Text, col); Text(fmt,
                        // ...); PopStyleColor();
    CHERRY_API void TextColoredV(const ImVec4 &col, const char *fmt, va_list args) IM_FMTLIST(2);
    CHERRY_API void TextDisabled(const char *fmt, ...) IM_FMTARGS(1);  // shortcut for PushStyleColor(ImGuiCol_Text,
                                                                       // style.Colors[ImGuiCol_TextDisabled]); Text(fmt,
                                                                       // ...); PopStyleColor();
    CHERRY_API void TextDisabledV(const char *fmt, va_list args) IM_FMTLIST(1);
    CHERRY_API void TextWrapped(const char *fmt, ...)
    ...
    CHERRY_API void LabelText(const char *label, const char *fmt, ...)
    ...
    CHERRY_API void BulletText(const char *fmt, ...) IM_FMTARGS(1);  // shortcut for Bullet()+Text()
    ...
    CHERRY_API bool IsItemVisible();               // is the last item visible? (items may be out
    ...
    CHERRY_API bool IsItemActivated();             // was the last item just made active (item
    ...
    CHERRY_API bool IsItemDeactivatedAfterEdit();  // was the last item just made inactive and
                                                   ...
                                                   // selected item).
    CHERRY_API bool IsItemToggledOpen();           // was the last item open state toggled?
                                                   // set by TreeNode().
    ...
    CHERRY_API ImVec2 GetItemRectMin();            // get upper-left bounding rectangle of the
                                                   // last item (screen space)
    CHERRY_API ImVec2 GetItemRectMax();            // get lower-right bounding rectangle of the
                                                   // last item (screen space)
    ...
    CHERRY_API void BeginHorizontal(const char *str_id, const ImVec2 &size = ImVec2(0, 0), float align = -1.0f);
    CHERRY_API void BeginHorizontal(const void *ptr_id, const ImVec2 &size = ImVec2(0, 0), float align = -1.0f);
    CHERRY_API void BeginHorizontal(int id, const ImVec2 &size = ImVec2(0, 0), float align = -1);
    CHERRY_API void EndHorizontal();
    CHERRY_API void BeginVertical(const char *str_id, const ImVec2 &size = ImVec2(0, 0), float align = -1.0f);
    CHERRY_API void BeginVertical(const void *ptr_id, const ImVec2 &size = ImVec2(0, 0), float align = -1.0f);
    ...
    CHERRY_API bool IsMouseHoveringRect(const ImVec2 &r_min, const ImVec2 &r_max, bool clip = true);
    ...
    CHERRY_API ImVec2 GetMouseDragDelta(ImGuiMouseButton button = 0, float lock_threshold = -1.0f);
    ...
    // Text
    CHERRY_API ImVec2 CalcTextSize(
        const char *text,
        const char *text_end = NULL,
    ...
    CHERRY_API bool BeginCombo(const char *label, const char *preview_value, ImGuiComboFlags flags = 0);
    CHERRY_API bool BeginCombo(const char *label, const std::function<void()> &preview, ImGuiComboFlags flags = 0);
    CHERRY_API void EndCombo();  // only call EndCombo() if BeginCombo() returns true!
    CHERRY_API bool Combo(
        const char *label,
        int *current_item,
    ...
    CHERRY_API bool IsWindowFocused(ImGuiFocusedFlags flags = 0);  // is current window focused? or its root/child,
                                                                   // depending on flags. see flags for options.
    CHERRY_API bool IsWindowHovered(
        ImGuiHoveredFlags flags = 0);               // is current window hovered (and typically: not blocked by a
                                                    ...
                                                    // Please read the FAQ!
    CHERRY_API ImDrawList *GetWindowDrawList();     // get draw list associated to the current window, to
                                                    // append your own drawing primitives
    CHERRY_API float GetWindowDpiScale();           // get DPI scale currently associated to
    ...
    CHERRY_API float GetWindowHeight();             // get current window height (shortcut for
                                                    // GetWindowSize().y)
    CHERRY_API ImGuiViewport *GetWindowViewport();  // get viewport currently associated to the current window.
    ...
    CHERRY_API void SetNextWindowFocus();                        // set next window to be focused /
                                                                 // top-most. call before Begin()
    CHERRY_API void SetNextWindowBgAlpha(float alpha);           // set next window background color alpha. helper to easily
    ...
    CHERRY_API void SetWindowFontScale(float scale);  // [OBSOLETE] set font scale. Adjust IO.FontGlobalScale if you
    ...
    CHERRY_API ImVec2 GetWindowContentRegionMin();  // content boundaries min for the full window
                                                    // (roughly (0,0)-Scroll), in window coordinates
    CHERRY_API ImVec2 GetWindowContentRegionMax();  // content boundaries max for the full window
    ...
    CHERRY_API void SetScrollHereX(float center_x_ratio = 0.5f);  // adjust scrolling amount to make current cursor position
    ...
    CHERRY_API void SetScrollHereY(float center_y_ratio = 0.5f);  // adjust scrolling amount to make current cursor position
    ...
    CHERRY_API void ColorConvertRGBtoHSV(float r, float g, float b, float &out_h, float &out_s, float &out_v);
    CHERRY_API void ColorConvertHSVtoRGB(float h, float s, float v, float &out_r, float &out_g, float &out_b);
    ...
    CHERRY_API ImGuiStyle &GetStyle();     // access the Style structure (colors, sizes). Always use
    ...
    CHERRY_API bool TreeNode(const char *str_id, const char *fmt, ...)
    ...
    CHERRY_API bool TreeNodeEx(const char *str_id, ImGuiTreeNodeFlags flags, const char *fmt, ...) IM_FMTARGS(3);
    CHERRY_API bool TreeNodeEx(const void *ptr_id, ImGuiTreeNodeFlags flags, const char *fmt, ...) IM_FMTARGS(3);
    CHERRY_API bool TreeNodeExV(const char *str_id, ImGuiTreeNodeFlags flags, const char *fmt, va_list args) IM_FMTLIST(3);
    CHERRY_API bool TreeNodeExV(const void *ptr_id, ImGuiTreeNodeFlags flags, const char *fmt, va_list args) IM_FMTLIST(3);
    CHERRY_API void TreePush(const char *str_id);         // ~ Indent()+PushId(). Already called by
    ...
    CHERRY_API bool CollapsingHeader(const char *label, ImTextureID *texture, ImGuiTreeNodeFlags flags);
    ...
    CHERRY_API void SetNextItemOpen(bool is_open,
    ...
    CHERRY_API void SetTooltip(const char *fmt, ...)
    ...
    CHERRY_API bool BeginListBox(const char *label, const ImVec2 &size = ImVec2(0, 0));  // open a framed scrolling region
    ...
    CHERRY_API void PushItemWidth(float item_width);  // push width of items for common large "item+label"
    ...
    CHERRY_API void SetNextItemWidth(float item_width);  // set width of the _next_ common large "item+label"
    ...
    CHERRY_API const char *GetKeyName(ImGuiKey key);  // [DEBUG] returns English name of the key. Those
    ...
    CHERRY_API void CaptureKeyboardFromApp(
        bool want_capture_keyboard_value = true);  // attention: misleading name! manually override
    ...
    CHERRY_API bool InputText(
        ...
        void *user_data = NULL);
    CHERRY_API bool InputTextWithImage(
        const char *label,
        ImTextureID texture,
        char *buf,
        size_t buf_size,
        ...
        ImGuiInputTextCallback callback = NULL,
        void *user_data = NULL);
    CHERRY_API bool InputTextMultiline(
        const char *label,
        char *buf,
        size_t buf_size,
        ...
        ImGuiInputTextCallback callback = NULL,
        void *user_data = NULL);
    CHERRY_API bool InputTextWithHint(
        const char *label,
        const char *hint,
        char *buf,
        size_t buf_size,
    ...
    CHERRY_API bool InputFloat(
        ...
        ImGuiInputTextFlags flags = 0);
    CHERRY_API bool InputColorFloat(
        const char *label,
        float *v,
        ...
        const char *format = "%.3f",
        ImGuiInputTextFlags flags = 0);
    CHERRY_API bool InputFloat2(const char *label, float v[2], const char *format = "%.3f", ImGuiInputTextFlags flags = 0);
    CHERRY_API bool InputFloat3(const char *label, float v[3], const char *format = "%.3f", ImGuiInputTextFlags flags = 0);
    CHERRY_API bool InputFloat4(const char *label, float v[4], const char *format = "%.3f", ImGuiInputTextFlags flags = 0);
    CHERRY_API bool InputInt(const char *label, int *v, int step = 1, int step_fast = 100, ImGuiInputTextFlags flags = 0);
    ...
    CHERRY_API bool InputDouble(
        ...
        ImGuiInputTextFlags flags = 0);
    CHERRY_API bool InputColorScalar(
        const char *label,
    ...
    CHERRY_API void SetItemDefaultFocus();                 // make last item the default focused
                                                           // item of a window.
    CHERRY_API void SetKeyboardFocusHere(int offset = 0);  // focus keyboard on the next widget. Use positive 'offset'
                                                           // to access sub components of a multiple component widget.
                                                           // Use -1 to access previous widget.
#ifndef IMGUI_DISABLE_OBSOLETE_KEYIO
    CHERRY_API int GetKeyIndex(ImGuiKey key);  // map ImGuiKey_* values into legacy
    ...
    CHERRY_API bool ColorPicker3(const char *label, float col[3], ImGuiColorEditFlags flags = 0);
    CHERRY_API bool
    ...
    CHERRY_API void
    RenderArrow(ImDrawList *draw_list, ImTextureID *texture, ImVec2 pos, ImU32 col, ImGuiDir dir, float scale);
    CHERRY_API void RenderArrow(ImDrawList *draw_list, ImVec2 pos, ImU32 col, ImGuiDir dir, float scale = 1.0f);
    CHERRY_API void RenderBullet(ImDrawList *draw_list, ImTextureID *texture, ImVec2 pos, ImU32 col);
    ...
    CHERRY_API void RenderArrowPointingAt(ImDrawList *draw_list, ImVec2 pos, ImVec2 half_sz, ImGuiDir direction, ImU32 col);
    CHERRY_API void RenderArrowDockMenu(ImDrawList *draw_list, ImVec2 p_min, float sz, ImU32 col);
    CHERRY_API void RenderRectFilledRangeH(
        ImDrawList *draw_list,
        const ImRect &rect,
        ImU32 col,
        float x_start_norm,
        float x_end_norm,
        float rounding);
    CHERRY_API void RenderRectFilledWithHole(ImDrawList *draw_list, ImRect outer, ImRect inner, ImU32 col, float rounding);
    CHERRY_API ImDrawFlags CalcRoundingFlagsForRectInRect(const ImRect &r_in, const ImRect &r_outer, float threshold);
    CHERRY_API int TableGetColumnCount();                          // return number of columns (value passed
                                                                   // to BeginTable)
    CHERRY_API int TableGetColumnIndex();                          // return current column index.
    CHERRY_API int TableGetRowIndex();                             // return current row index.
    CHERRY_API const char *TableGetColumnName(int column_n = -1);  // return "" if column didn't have a name declared by
                                                                   // TableSetupColumn(). Pass -1 to use current column.
    CHERRY_API ImGuiTableColumnFlags TableGetColumnFlags(int column_n = -1);  // return column flags so you can query their
    ...
    CHERRY_API bool DragFloat4(
        ...
        const char *format = "%.3f",
        ImGuiSliderFlags flags = 0);
    CHERRY_API bool DragFloatRange2(
        const char *label,
        float *v_current_min,
        float *v_current_max,
        float v_speed = 1.0f,
    ...
    CHERRY_API void NextColumn();                            // next column, defaults to current row or next
    ...
    CHERRY_API void SetColumnOffset(
        int column_index,
    ...
    CHERRY_API void TableSetupColumn(
        const char *label,
        ImGuiTableColumnFlags flags = 0,
        float init_width_or_weight = 0.0f,
        ImGuiID user_id = 0);
    CHERRY_API void TableSetupScrollFreeze(int cols,
    ...
    CHERRY_API void TableHeader(const char *label);    // submit one header cell manually (rarely used)

    CHERRY_API bool BeginTable(
        const char *str_id,
        int column,
    ...
    CHERRY_API void TableSetColumnSortDirection(int column_n, ImGuiSortDirection sort_direction, bool append_to_sort_specs);
    ...
    CHERRY_API void PushAllowKeyboardFocus(bool allow_keyboard_focus);  // == tab stop enable. Allow focusing using
                                                                        // TAB/Shift-TAB, enabled by default but you can
                                                                        // disable it for certain widgets
    CHERRY_API void PopAllowKeyboardFocus();

    CHERRY_API bool IsPopupOpen(const char *str_id,
