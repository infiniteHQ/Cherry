//
//  app_window.cpp
//  Sources for app windows of cherry
//
...
namespace Cherry {
  ...
  AppWindow::AppWindow(const std::string &id, const std::string &name, const std::string &icon)
      : m_ID(id),
        m_Name(name),
        m_Icon(icon) {
    SetIcon(m_Icon);
      ...
      for (auto it = reqs->begin(); it != reqs->end();) {
        ...
        switch (req->m_DockPlace) {
          ...
          case DockEmplacement::DockRight:
            ...
            break;
          case DockEmplacement::DockFull: m_DockSpaceID = parentDockID; break;
          default: ++it; continue;
        }
