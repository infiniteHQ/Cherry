...
namespace Cherry {
  ...
  static int ThirdMain(int argc, char **argv, Cherry::Application *(*create_app)(int, char **)) {
    while (Cherry::Application::RunningState().load(std::memory_order_acquire)) {
      Cherry::Application *app = create_app(argc, argv);

      switch (app->GetDefaultSpecification().RuntimeMode) {
        case Runtime::SingleThread:
        default: app->SingleThreadRuntime(); break;
      }
