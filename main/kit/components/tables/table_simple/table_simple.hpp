...
namespace Cherry {
  ...
  // End-User API
  namespace Kit {
    inline Component &TableSimple(
        const Identifier &identifier,
        const std::string &label,
        const std::vector<Component> &rows = {},
        ImGuiTableFlags flags = 0) {
      return CherryApp.PushComponent<Cherry::Components::TableSimple>(identifier, label, rows, flags);
    }

    inline Component &
    TableSimple(const std::string &label, const std::vector<Component> &rows = {}, ImGuiTableFlags flags = 0) {
      return Cherry::Kit::TableSimple(Application::GenerateUniqueID(label, rows, flags, "TableSimple"), label, rows, flags);
    }
