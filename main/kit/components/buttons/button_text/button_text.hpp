...
namespace Cherry {
  namespace Components {
    class ButtonText : public Component {
     public:
      ButtonText(const Cherry::Identifier &id, const std::string &label) : Component(id) {
        // Identifier
        SetIdentifier(id);
      ...
      void Render() override {
        const ImVec2 &size = ImVec2(GetPropertyAs<int>("size_x"), GetPropertyAs<int>("size_y"));
  ...
  namespace Kit {
    ...
    inline Component &ButtonText(const Identifier &identifier, const std::string &label) {
      return CherryApp.PushComponent<Cherry::Components::ButtonText>(identifier, label);
    }

    inline Component &ButtonText(const std::string &label) {
      return Cherry::Kit::ButtonText(Application::GenerateUniqueID(label, "ButtonText"), label);
    }
