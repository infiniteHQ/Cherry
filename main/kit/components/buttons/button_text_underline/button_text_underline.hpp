...
namespace Cherry {
  namespace Components {
    class ButtonTextUnderline : public Component {
     public:
      ButtonTextUnderline(const Cherry::Identifier &id, const std::string &label) : Component(id) {
        // Identifier
        SetIdentifier(id);

        // Colors
        SetProperty("color_underline", "theme:button_color_underline");
        SetProperty("color_underline_hovered", "theme:button_color_underline_hovered");
        SetProperty("color_underline_pressed", "theme:button_color_underline_pressed");
      ...
      void Render() override {
        ...
        bool isClicked = CherryGUI::Button(
            GetProperty("label").c_str(), ImVec2(std::stoi(GetProperty("size_x")), std::stoi(GetProperty("size_y"))));
        ...
        if (CherryGUI::IsItemHovered()) {
          ...
          ImVec4 underline_color =
              CherryGUI::IsItemActive() ? HexToRGBA(GetProperty("color_text")) : HexToRGBA(GetProperty("color_text"));
          draw_list->AddLine(
              ImVec2(pos.x, pos.y + rect_size.y),
  ...
  // End-User API
  namespace Kit {
    inline Component &ButtonTextUnderline(const Identifier &identifier, const std::string &label) {
      return CherryApp.PushComponent<Cherry::Components::ButtonTextUnderline>(identifier, label);
    }

    inline Component &ButtonTextUnderline(const std::string &label) {
      return Cherry::Kit::ButtonTextUnderline(Application::GenerateUniqueID(label), label);
    }
