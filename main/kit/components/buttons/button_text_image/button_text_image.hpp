...
namespace Cherry {
  ...
  namespace Kit {
    ...
    inline Component &
    ButtonTextImage(const Identifier &identifier, const std::string &label, const std::string &image_path) {
      return CherryApp.PushComponent<Cherry::Components::ButtonTextImage>(identifier, label, image_path);
    }

    inline Component &ButtonTextImage(const std::string &label, const std::string &image_path) {
      return Cherry::Kit::ButtonTextImage(
          Application::GenerateUniqueID(label, image_path, "ButtonTextImage"), label, image_path);
    }
  }  // namespace Kit
