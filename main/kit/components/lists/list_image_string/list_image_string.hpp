...
namespace Cherry {
  ...
  // End-User API
  namespace Kit {
    inline Component &ListImageString(
        const Identifier &identifier,
        const std::string &label,
        const std::vector<std::pair<std::string, std::string>> &values,
        int default_selected = 0) {
      return CherryApp.PushComponent<Cherry::Components::ListImageString>(identifier, label, values, default_selected);
    }
    ...
    inline Component &ListImageString(
      ...
      return Cherry::Kit::ListImageString(
          Application::GenerateUniqueID(label, values, default_selected, "ListImageString"),
          label,
          values,
          default_selected);
    }
