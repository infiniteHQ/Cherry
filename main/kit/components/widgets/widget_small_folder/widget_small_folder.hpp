...
namespace Cherry {
  ...
  // End-User API
  namespace Kit {
    inline Component &WidgetSmallFolder(
        const Identifier &identifier,
        const std::string &color = "#CC9900",
        const float &size_x = 40.0f,
        const float &size_y = 40.0f) {
      return CherryApp.PushComponent<Cherry::Components::WidgetSmallFolder>(identifier, color, size_x, size_y);
    }

    inline Component &
    WidgetSmallFolder(const std::string &color = "#CC9900", const float &size_x = 40.0f, const float &size_y = 40.0f) {
      return Cherry::Kit::WidgetSmallFolder(
          Application::GenerateUniqueID(color, size_x, size_y, "WidgetSmallFolder"), color, size_x, size_y);
    }
