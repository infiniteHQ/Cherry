...
namespace Cherry {
  namespace Components {
    class TitleFive : public Component {
     public:
      TitleFive(const Cherry::Identifier &id, const std::string &label) : Component(id) {
        // Identifier
        SetIdentifier(id);
  ...
  // End-User API
  namespace Kit {
    inline Component &TitleFive(const Identifier &identifier, const std::string &label) {
      return CherryApp.PushComponent<Cherry::Components::TitleFive>(identifier, label);
    }

    inline Component &TitleFive(const std::string &label) {
      return Cherry::Kit::TitleFive(Application::GenerateUniqueID(label, "TitleFive"), label);
    }
