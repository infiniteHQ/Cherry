...
namespace Cherry {
  ...
  // End-User API
  namespace Kit {
    inline Component &TitleSix(const Identifier &identifier, const std::string &label) {
      return CherryApp.PushComponent<Cherry::Components::TitleSix>(identifier, label);
    }

    inline Component &TitleSix(const std::string &label) {
      return Cherry::Kit::TitleSix(Application::GenerateUniqueID(label, "TitleSix"), label);
    }
