...
namespace Cherry {
  ...
  namespace Kit {
    inline Component &
    TooltipImage(const Identifier &identifier, const std::string &image_path, const std::string &description) {
      return CherryApp.PushComponent<Cherry::Components::TooltipImage>(identifier, image_path, description);
    }

    inline Component &TooltipImage(const std::string &image_path, const std::string &description) {
      return Cherry::Kit::TooltipImage(
          Application::GenerateUniqueID(image_path, description, "TooltipImage"), image_path, description);
    }
