...
namespace Cherry {
  ...
  // End-User API
  namespace Kit {
    inline Component &TooltipText(const Identifier &identifier, const std::string &label, const std::string &description) {
      return CherryApp.PushComponent<Cherry::Components::TooltipText>(identifier, label, description);
    }

    inline Component &TooltipText(const std::string &label, const std::string &description) {
      return Cherry::Kit::TooltipText(Application::GenerateUniqueID(label, description, "TooltipText"), label, description);
    }
