...
namespace Cherry {
      ...
      void Render() override {
        ...
        if (m_Value) {
          ...
          for (int len = (int)description.size(); len >= 0; --len) {
            std::string test = (len < (int)description.size()) ? description.substr(0, len) + ellipsis : description;
          ...
          if (std::strlen(buffer) == 0) {
            float hintY = cursorPos.y + padding.y +
                          (CherryGUI::GetFontSize() - CherryGUI::CalcTextSize(displayedHint.c_str()).y) * 0.5f;

            ImVec2 clipMin = ImVec2(textX, cursorPos.y);
            ImVec2 clipMax = ImVec2(textX + availableTextWidth, cursorPos.y + CherryGUI::GetFontSize() + padding.y * 2.0f);

            CherryGUI::PushClipRect(clipMin, clipMax, true);
            CherryGUI::GetWindowDrawList()->AddText(
                ImVec2(textX, hintY), CherryGUI::GetColorU32(ImGuiCol_TextDisabled), displayedHint.c_str());
            CherryGUI::PopClipRect();

            if (hasLogo) {
              float logoY = cursorPos.y + padding.y + (CherryGUI::GetFontSize() - logoSize) * 0.5f;
              float logoX = (logoPlacement == "l") ? cursorPos.x + padding.x : cursorPos.x + sizeX - padding.x - logoSize;

              CherryGUI::GetWindowDrawList()->AddImage(
                  logo, ImVec2(logoX, logoY), ImVec2(logoX + logoSize, logoY + logoSize));
            }
          }
  ...
  // End-User API
  namespace Kit {
    inline Component &InputString(const Identifier &identifier, const std::string &label, std::string *value) {
      return CherryApp.PushComponent<Cherry::Components::InputString>(identifier, label, value);
    }

    inline Component &InputString(const std::string &label, std::string *value) {
      return Cherry::Kit::InputString(Application::GenerateUniqueID(label, value, "InputString"), label, value);
    }
