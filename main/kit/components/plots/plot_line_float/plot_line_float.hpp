...
namespace Cherry {
  ...
  // End-User API
  namespace Kit {
    inline Component &PlotLineFloat(
        const Identifier &identifier,
        const std::string &label,
        const std::vector<float> &float_table,
        int number_showed = -1) {
      return CherryApp.PushComponent<Cherry::Components::PlotLineFloat>(identifier, label, float_table, number_showed);
    }

    inline Component &
    PlotLineFloat(const std::string &label, const std::vector<float> &float_table, int number_showed = -1) {
      return Cherry::Kit::PlotLineFloat(
          Application::GenerateUniqueID(label, float_table, number_showed, "PlotLineFloat"),
          label,
          float_table,
          number_showed);
    }
    ...
    inline Component &PlotLineFloat(const std::string &label, const std::vector<float> &float_table, int *number_showed) {
      return Cherry::Kit::PlotLineFloat(
          Application::GenerateUniqueID(label, float_table, number_showed, "PlotLineFloat"),
          label,
          float_table,
