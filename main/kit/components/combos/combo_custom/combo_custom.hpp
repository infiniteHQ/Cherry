...
namespace Cherry {
    ...
    class ComboCustom : public Component {
     public:
      ...
      void Render() override {
        ...
        if (CherryGUI::BeginCombo(Label.c_str(), [&]() { this->m_List[selected]->Render(); })) {
  ...
  // End-User API
  namespace Kit {
    inline Component &ComboCustom(
        const Identifier &identifier,
        const std::string &label,
        const std::vector<Component> &list,
        int default_index = 0) {
      return CherryApp.PushComponent<Cherry::Components::ComboCustom>(identifier, label, list, default_index);
    }

    inline Component &ComboCustom(const std::string &label, const std::vector<Component> &list, int default_index = 0) {
      return Cherry::Kit::ComboCustom(
          Application::GenerateUniqueID(label, list, default_index, "ComboCustom"), label, list, default_index);
    }
