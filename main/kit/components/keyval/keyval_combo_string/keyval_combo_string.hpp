...
namespace Cherry {
  ...
  // End-User API
  namespace Kit {
    inline Component &KeyValComboString(
        const Identifier &identifier,
        const std::string &label,
        std::vector<std::string> *list,
        int default_selected_index = 0) {
      return CherryApp.PushComponent<Cherry::Components::KeyValComboString>(identifier, label, list, default_selected_index);
    }

    inline Component &
    KeyValComboString(const std::string &label, std::vector<std::string> *list, int default_selected_index = 0) {
      return Cherry::Kit::KeyValComboString(
          Application::GenerateUniqueID(label, list, default_selected_index, "KeyValComboString"),
          label,
          list,
