...
namespace Cherry {
  ...
  namespace Kit {
    inline Component &
    KeyValCustom(const Identifier &identifier, const std::string &label, const std::function<void()> &renderCallback) {
      return CherryApp.PushComponent<Cherry::Components::KeyValCustom>(identifier, label, renderCallback);
    }

    inline Component &KeyValCustom(const std::string &label, const std::function<void()> &renderCallback) {
      return Cherry::Kit::KeyValCustom(
          Application::GenerateUniqueID(label, renderCallback, "KeyValCustom"), label, renderCallback);
    }
