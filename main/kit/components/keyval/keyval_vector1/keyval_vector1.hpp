...
namespace Cherry {
  ...
  // End-User API
  namespace Kit {
    inline Component &KeyValVector1(const Identifier &identifier, const std::string &label, float *x_value) {
      return CherryApp.PushComponent<Cherry::Components::KeyValVector1>(identifier, label, x_value);
    }

    inline Component &KeyValVector1(const std::string &label, float *x_value) {
      return Cherry::Kit::KeyValVector1(Application::GenerateUniqueID(label, x_value, "KeyValVector1"), label, x_value);
    }
