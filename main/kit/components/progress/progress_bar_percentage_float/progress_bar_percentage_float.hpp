...
namespace Cherry {
  ...
  // End-User API
  namespace Kit {
    inline Component &ProgressBarPercentageFloat(const Identifier &identifier, float *number, float *goal) {
      return CherryApp.PushComponent<Cherry::Components::ProgressBarPercentageFloat>(identifier, number, goal);
    }

    inline Component &ProgressBarPercentageFloat(float *number, float *goal) {
      return Cherry::Kit::ProgressBarPercentageFloat(
          Application::GenerateUniqueID(number, goal, "ProgressBarPercentageFloat"), number, goal);
    }
