...
namespace Cherry {
  ...
  // End-User API
  namespace Kit {
    inline Component &ProgressBarPercentageInteger(const Identifier &identifier, int *number, int *goal) {
      return CherryApp.PushComponent<Cherry::Components::ProgressBarPercentageInteger>(identifier, number, goal);
    }

    inline Component &ProgressBarPercentageInteger(int *number, int *goal) {
      return Cherry::Kit::ProgressBarPercentageInteger(
          Application::GenerateUniqueID(number, goal, "ProgressBarPercentageInteger"), number, goal);
    }
