...
namespace Cherry {
  ...
  // End-User API
  namespace Kit {
    inline Component &ImageLocalCentered(
        const Identifier &identifier,
        const std::string &image_path,
        float x_size = 50.0f,
        float y_size = 50.0f) {
      return CherryApp.PushComponent<Cherry::Components::ImageLocalCentered>(identifier, image_path, x_size, y_size);
    }

    inline Component &ImageLocalCentered(const std::string &image_path, float x_size = 50.0f, float y_size = 50.0f) {
      return Cherry::Kit::ImageLocalCentered(
          Application::GenerateUniqueID(image_path, x_size, y_size, "ImageLocalCentered"), image_path, x_size, y_size);
    }
