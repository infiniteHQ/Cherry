...
namespace Cherry {
  ...
  namespace Kit {
    inline Component &
    ImageHttp(const Identifier &identifier, const std::string &image_http_path, float x_size = 50.0f, float y_size = 50.0f) {
      return CherryApp.PushComponent<Cherry::Components::ImageHttp>(identifier, image_http_path, x_size, y_size);
    }

    inline Component &ImageHttp(const std::string &image_http_path, float x_size = 50.0f, float y_size = 50.0f) {
      return Cherry::Kit::ImageHttp(
          Application::GenerateUniqueID(image_http_path, x_size, y_size, "ImageHttp"), image_http_path, x_size, y_size);
    }
