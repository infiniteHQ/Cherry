...
namespace Cherry {
  ...
  namespace Kit {
    inline Component &ModalSimple(
      ...
      return CherryApp.PushComponent<Cherry::Components::ModalSimple>(identifier, title, activated, render_callback);
    }

    inline Component &
    ModalSimple(const std::string &title, bool *activated, const std::function<void()> &render_callback = []() { }) {
      return Cherry::Kit::ModalSimple(
          Application::GenerateUniqueID(title, activated, render_callback, "ModalSimple"),
          title,
          activated,
          render_callback);
    }
