...
namespace Cherry {
  namespace Components {
    class TextSimple : public Component {
     public:
      TextSimple(const Cherry::Identifier &id, const std::string &label) : Component(id) {
        // Identifier
        SetIdentifier(id);
  ...
  // End-User API
  namespace Kit {
    inline Component &TextSimple(const Identifier &identifier, const std::string &label) {
      return CherryApp.PushComponent<Cherry::Components::TextSimple>(identifier, label);
    }

    inline Component &TextSimple(const std::string &label) {
      return Cherry::Kit::TextSimple(Application::GenerateUniqueID(label + "TextSimple"), label);
    }
