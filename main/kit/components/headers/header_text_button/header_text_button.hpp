...
namespace Cherry {
  namespace Components {
    class HeaderTextButton : public Component {
     public:
      HeaderTextButton(const Cherry::Identifier &id, const std::string &label, const std::function<void()> &render_content)
          : Component(id),
            m_RenderContent(render_content) {
        // Identifier
        SetIdentifier(id);

        // Colors
        SetProperty("button_color_border", "theme:header_button_color_border");
        SetProperty("button_color_border_hovered", "theme:header_button_color_border_hovered");
        SetProperty("button_color_border_clicked", "theme:header_button_color_border_clicked");
        SetProperty("button_color_bg", "theme:header_button_color_bg");
        SetProperty("button_color_bg_hovered", "theme:header_button_color_bg_hovered");
        SetProperty("button_color_bg_clicked", "theme:header_button_color_bg_clicked");
      ...
      void Render() override {
        ...
        ImVec2 arrow_pos = ImVec2(
            CherryGUI::GetItemRectMax().x - style.FramePadding.x - CherryGUI::GetFontSize(),
            CherryGUI::GetItemRectMin().y + style.FramePadding.y + 3.0f);
        CherryGUI::RenderArrow(
            CherryGUI::GetWindowDrawList(),
            arrow_pos,
            CherryGUI::GetColorU32(ImGuiCol_Text),
            GetData("isOpened") == "true" ? ImGuiDir_Down : ImGuiDir_Right);
  ...
  namespace Kit {
    inline Component &HeaderTextButton(
      ...
      return CherryApp.PushComponent<Cherry::Components::HeaderTextButton>(identifier, label, render_content);
    }

    inline Component &HeaderTextButton(const std::string &label, const std::function<void()> &render_content = []() { }) {
      return Cherry::Kit::HeaderTextButton(
          Application::GenerateUniqueID(label, render_content, "HeaderTextButton"), label, render_content);
    }
  }  // namespace Kit
}  // namespace Cherry
