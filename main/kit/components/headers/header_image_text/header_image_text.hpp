...
namespace Cherry {
  ...
  namespace Kit {
    ...
    inline Component &HeaderImageText(
        ...
        const std::string &image_path,
        const std::function<void()> &render_content = []() { }) {
      return CherryApp.PushComponent<Cherry::Components::HeaderImageText>(identifier, label, image_path, render_content);
    }
    ...
    inline Component &HeaderImageText(
      ...
      return Cherry::Kit::HeaderImageText(
          Application::GenerateUniqueID(label, image_path, render_content, "HeaderImageText"),
          label,
          image_path,
          render_content);
    }
