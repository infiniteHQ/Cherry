...
namespace Cherry {
  ...
  // End-User API
  namespace Kit {
    inline Component &TextArea(const Identifier &identifier, float *width, float *height, const std::string &buffer) {
      return CherryApp.PushComponent<Cherry::Components::TextArea>(identifier, width, height, buffer);
    }

    inline Component &TextArea(float *width, float *height, const std::string &buffer) {
      return Cherry::Kit::TextArea(Application::GenerateUniqueID(width, height, buffer, "TextArea"), width, height, buffer);
    }
