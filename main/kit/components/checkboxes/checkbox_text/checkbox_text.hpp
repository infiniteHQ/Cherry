...
namespace Cherry {
  ...
  // End-User API
  namespace Kit {
    inline Component &CheckboxText(const Identifier &identifier, const std::string &label, bool *checked) {
      return CherryApp.PushComponent<Cherry::Components::CheckboxText>(identifier, label, checked);
    }

    inline Component &CheckboxText(const std::string &label, bool *checked) {
      return Cherry::Kit::CheckboxText(Application::GenerateUniqueID(label, checked), label, checked);
    }
  }  // namespace Kit
}  // namespace Cherry
