...
//
// UIKit's logging system is based on the spdlog logging library (https://github.com/gabime/spdlog)
// and is pretty much a copy of Hazel's logging system.
// All HZ_ macros are replaced with WL_ for UIKit, and Hazel namespaces have been changed to UIKit.
// 

#include "log_custom_formatters.hpp"
...
namespace UIKit {
	...
	void Log::PrintAssertMessage(Log::Type type, std::string_view prefix, Args&&... args)
	{
		...
		MessageBoxA(nullptr, message.c_str(), "UIKit Assert", MB_OK | MB_ICONERROR);
#endif
	}
...
#if WL_ASSERT_MESSAGE_BOX
		MessageBoxA(nullptr, "No message :(", "UIKit Assert", MB_OK | MB_ICONERROR);
#endif
	}
}
