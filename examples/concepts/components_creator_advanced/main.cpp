     ...
     public:
      OrbitalVisualizer(const Cherry::Identifier &id) : Component(id) {
        ...
        for (int i = 0; i < m_Count; i++) {
          m_Particles.push_back({ (float)i * (6.28f / m_Count), 20.0f + (i * 0.5f), 0.5f + ((float)rand() / RAND_MAX) });
        }
      }
      ...
      void Render() override {
        ...
        ImU32 color = CherryGUI::ColorConvertFloat4ToU32(HexToRGBA(GetProperty("color")));
